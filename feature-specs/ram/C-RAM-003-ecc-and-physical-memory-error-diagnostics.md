# Feature Specification — C-RAM-003 — ECC & physical-memory error diagnostics

## 3. Identificação

Nome: ECC & physical-memory error diagnostics
ID: C-RAM-003
...
**Base conceitual do Discovery:** [25] ECC Capability Detection — `CANDIDATE`; [26] ECC Error Monitoring — `CANDIDATE`; [27] Predictive Bad Memory Page Detection — `IDEA`; [127] Memory Fault Source Classification — `IDEA`; [136] Memory Error Address Pattern Analysis — `IDEA`
...
Evita decisões baseadas em suposição sobre **ECC & physical-memory error diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for ECC & physical-memory error diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **ECC & physical-memory error diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **ECC & physical-memory error diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/ram/c-ram-003/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — ECC capability/erro e `Unsupported`.
- Evidência realmente executada: NENHUMA.
- Execução segura neste host / teste: Sim para negativa — consultar WHEA/ECC properties e registrar não-ECC/Unknown.
- Impedimento ou limitação restante: Sem RAM ECC ou erro físico; não induzir erro.
