# Feature Specification — C-RAM-016 — Large-page capability & application guidance

## 3. Identificação

Nome: Large-page capability & application guidance
ID: C-RAM-016
Tipo: Diagnostic, Diagnostic / Configuration Guidance
...
**Base conceitual do Discovery:** [113] Large Page Capability Detection — `CANDIDATE`; [114] Large Page Application Usage Detection — `RESEARCHING`; [116] Large Page Application Guidance — `IDEA`
...
Evita decisões baseadas em suposição sobre **Large-page capability & application guidance**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Large-page capability & application guidance; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
Target state: Only an explicitly selected, supported change for **Large-page capability & application guidance**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/ram/c-ram-016/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Large-page capability can be detected without changing SeLockMemoryPrivilege or allocating a large page.
...
- Limites preservados: privileged allocation intentionally not attempted
...
- Prova prática adicional essencial: Sim — grande página/privilege e falha explícita.
- Evidência realmente executada: Prototype seguro adicional executado: Large-page capability can be detected without changing SeLockMemoryPrivilege or allocating a large page. Resultado observado: `PARTIAL`. Evidência: `/prototypes/ram/C-RAM-004/results/ram-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — P/Invoke `GetLargePageMinimum`; tentativa limitada `VirtualAlloc(MEM_LARGE_PAGES)` deve retornar falta de privilege sem mudar policy.
- Impedimento ou limitação restante: Não há `SeLockMemoryPrivilege`; sucesso precisa ambiente privilegiado.
