# Feature Specification — C-RAM-017 — Memory-bound workload diagnostics
...
**Base conceitual do Discovery:** [129] Memory-Bound Workload Detection — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Memory-bound workload diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Memory-bound workload diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Memory-bound workload diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **Memory-bound workload diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/ram/c-ram-017/` — **TBD / ainda não executado nesta fase documental**.
