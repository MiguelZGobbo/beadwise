# Feature Specification — C-RAM-006 — Per-process memory & leak diagnostics

## 3. Identificação

Nome: Per-process memory & leak diagnostics
ID: C-RAM-006
...
**Base conceitual do Discovery:** [42] Working Set Monitoring — `CANDIDATE`; [44] Process Memory Growth Detection — `CANDIDATE`; [45] Memory Leak Candidate Detection — `CANDIDATE`; [64] Memory Usage by Process Ranking — `CANDIDATE`; [65] Memory Usage Anomaly Detection — `IDEA`
...
Evita decisões baseadas em suposição sobre **Per-process memory & leak diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Per-process memory & leak diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Per-process memory & leak diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **Per-process memory & leak diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/ram/c-ram-006/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Per-process growth and release can be sampled without classifying every retained working-set page as a leak.
...
- Limites preservados: allocator may retain pages; no real leaking process
...
- Prova prática adicional essencial: Sim — crescimento e classificação de leak.
- Evidência realmente executada: Prototype seguro adicional executado: Per-process growth and release can be sampled without classifying every retained working-set page as a leak. Resultado observado: `PARTIAL`. Evidência: `/prototypes/ram/C-RAM-004/results/ram-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — filho próprio aloca/libera buffers limitados em ciclos; amostrar WorkingSet/PrivateBytes e verificar retorno.
- Impedimento ou limitação restante: Não prova leak de processo real/kernel.
