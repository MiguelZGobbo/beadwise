# Feature Specification — C-RAM-015 — Memory-controller, voltage & training context
...
**Base conceitual do Discovery:** [90] Memory Controller Ratio Detection — `RESEARCHING`; [91] Intel Memory Gear Mode Detection — `CANDIDATE`; [92] Intel Gear Mode Performance Context — `CANDIDATE`; [93] Intel Gear Mode Modification — `QUESTIONABLE`; [94] AMD MCLK Detection — `RESEARCHING`; [95] AMD UCLK Detection — `RESEARCHING`; [96] AMD Fabric Clock Context — `RESEARCHING`; [101] DDR5 PMIC Information — `IDEA`; [102] Memory Voltage Detection — `RESEARCHING`; [103] Memory Training State Context — `IDEA`; [104] Memory Training Failure / Fallback Detection — `RESEARCHING`; [105] Memory Context Restore Awareness — `IDEA`; Limite conceitual:; O `READY_FOR_SPEC` vale para a responsabilidade principal descrita nesta capability. Subitens de origem marcados `QUESTIONABLE` continuam questionáveis e **não** são aprovados implicitamente; qualquer alteração agressiva neles representada permanece fora do escopo ativo até justificativa própria.
...
Evita decisões baseadas em suposição sobre **Memory-controller, voltage & training context**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Memory-controller, voltage & training context; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Memory-controller, voltage & training context** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **Memory-controller, voltage & training context** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/ram/c-ram-015/` — **TBD / ainda não executado nesta fase documental**.
