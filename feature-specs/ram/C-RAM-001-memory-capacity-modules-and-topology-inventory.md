# Feature Specification — C-RAM-001 — Memory capacity, modules & topology inventory

## 3. Identificação

Nome: Memory capacity, modules & topology inventory
ID: C-RAM-001
Tipo: Diagnostic, Diagnostic / Product Logic, Diagnostic / Recommendation, Product Behavior
...
**Base conceitual do Discovery:** [1] Physical Memory Capacity Detection — `CANDIDATE`; [2] OS-Usable Memory Detection — `CANDIDATE`; [3] Installed vs Usable RAM Difference Analysis — `CANDIDATE`; [4] Memory Module Inventory — `CANDIDATE`; [5] Memory Slot Population Detection — `CANDIDATE`; [6] Asymmetric Module Configuration Detection — `CANDIDATE`; [7] Memory Type Detection — `CANDIDATE`; [8] Memory Data Rate Detection — `CANDIDATE`; [9] Memory Clock vs Effective Data Rate Explanation — `CANDIDATE`; [21] Memory Channel Topology Detection — `CANDIDATE`; [22] Suboptimal DIMM Slot Placement Detection — `CANDIDATE`; [23] Channel Imbalance Detection — `IDEA`; [24] Rank Detection — `RESEARCHING`; [88] DDR5 Architecture Awareness — `CANDIDATE`; [89] DDR5 Subchannel Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Memory capacity, modules & topology inventory**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Memory capacity, modules & topology inventory; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Memory capacity, modules & topology inventory** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **Memory capacity, modules & topology inventory** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/ram/c-ram-001/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: CIM module/array inventory can be reconciled with native physical-memory total.
...
- Limites preservados: single desktop; no ECC or empty-slot matrix
...
- Prova prática adicional essencial: Sim — módulos/topologia e campos ausentes.
- Evidência realmente executada: Prototype seguro adicional executado: CIM module/array inventory can be reconciled with native physical-memory total. Resultado observado: `PASS`. Evidência: `/prototypes/ram/C-RAM-004/results/ram-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — comparar `Win32_PhysicalMemory`/`Win32_PhysicalMemoryArray`/`Win32_ComputerSystem` e normalizar dois DIMMs Kingston.
- Impedimento ou limitação restante: Sem slots vazios, ECC ou múltiplas plataformas.
