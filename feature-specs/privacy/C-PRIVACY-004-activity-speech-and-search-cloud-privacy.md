# Feature Specification — C-PRIVACY-004 — Activity, speech & search/cloud privacy

## 3. Identificação

Nome: Activity, speech & search/cloud privacy  
ID: C-PRIVACY-004  
...
**Base conceitual do Discovery:** [9] Activity History Controls — `CANDIDATE`; [10] Speech / Online Speech Recognition Privacy — `CANDIDATE`; [12] Search / Cloud Content Privacy Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Activity, speech & search/cloud privacy**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Activity, speech & search/cloud privacy; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Activity, speech & search/cloud privacy**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-PRIVACY-004
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Activity, speech & search/cloud privacy**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/privacy/c-privacy-004/` — **TBD / ainda não executado nesta fase documental**.
