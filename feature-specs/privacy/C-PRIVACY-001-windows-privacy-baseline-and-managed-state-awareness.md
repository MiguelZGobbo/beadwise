# Feature Specification — C-PRIVACY-001 — Windows privacy baseline & managed-state awareness

## 3. Identificação

Nome: Windows privacy baseline & managed-state awareness
ID: C-PRIVACY-001
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [1] Windows Privacy Baseline Inventory — `CANDIDATE`; [14] Privacy Policy / Managed State Awareness — `CANDIDATE`; [18] Per-User Privacy Scope Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Windows privacy baseline & managed-state awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- effective user setting plus managed-policy state; policy must win when enforced
- diagnostic-data and personalization policies through documented Policy CSP/Group Policy mappings
- app capability permissions through documented Privacy Policy CSP where applicable
- location/search/activity/Find My Device state using documented policy surfaces and user-control distinction
- N/A
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Windows privacy baseline & managed-state awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Windows privacy baseline & managed-state awareness** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
- Policy CSP - Search
- Group Policy/ADMX mappings documented by Microsoft
...
```text
Feature: C-PRIVACY-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Windows privacy baseline & managed-state awareness** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### APIs
Policy CSP - Privacy, Policy CSP - Experience, Policy CSP - Search, Group Policy/ADMX mappings documented by Microsoft

### Componentes do Windows
Componentes do subsistema PRIVACY e infraestrutura comum de Event Log/ETW/CIM quando aplicável.
...
### Script/protótipo
Local: `/prototypes/privacy/c-privacy-001/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Policy presence, scope and effective-state uncertainty can be reported without exporting values or identity.
...
- Limites preservados: no MDM/GPO conflict host; effective runtime behavior not independently queried
...
- Prova prática adicional essencial: Sim — effective/user/policy state e Managed/Unknown.
- Evidência realmente executada: Prototype seguro adicional executado: Policy presence, scope and effective-state uncertainty can be reported without exporting values or identity. Resultado observado: `PARTIAL`. Evidência: `/prototypes/privacy/C-PRIVACY-001/results/privacy-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — consulta Registry HKLM/HKCU documentado, comparar policy presence com valores efetivos sem mudar qualquer setting.
- Impedimento ou limitação restante: Só host pessoal; não há MDM/GPO conflitante.
