# Feature Specification — C-PRIVACY-005 — Find My Device privacy trade-off

## 3. Identificação

Nome: Find My Device privacy trade-off  
ID: C-PRIVACY-005  
...
**Base conceitual do Discovery:** [13] Find My Device Privacy Trade-off — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Find My Device privacy trade-off**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Find My Device privacy trade-off; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Find My Device privacy trade-off**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-PRIVACY-005
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Find My Device privacy trade-off**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/privacy/c-privacy-005/` — **TBD / ainda não executado nesta fase documental**.
