# Feature Specification — C-PRIVACY-003 — Location & sensitive-device permissions

## 3. Identificação

Nome: Location & sensitive-device permissions  
ID: C-PRIVACY-003  
...
**Base conceitual do Discovery:** [5] Location Privacy & App Access — `CANDIDATE`; [6] Camera Permission Inventory — `CANDIDATE`; [7] Microphone Permission Inventory — `CANDIDATE`; [8] App Permission Inventory — `CANDIDATE`; [16] Recent Sensitive Resource Access Review — `CANDIDATE`; [17] Desktop App vs Packaged App Permission Distinction — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Location & sensitive-device permissions**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Location & sensitive-device permissions; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Location & sensitive-device permissions**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-PRIVACY-003
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Location & sensitive-device permissions**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/privacy/c-privacy-003/` — **TBD / ainda não executado nesta fase documental**.
