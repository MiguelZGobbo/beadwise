# Feature Specification — C-ENERGY-004 — Energy efficiency & performance-per-watt analysis

## 3. Identificação

Nome: Energy efficiency & performance-per-watt analysis
ID: C-ENERGY-004
...
**Base conceitual do Discovery:** [20] Idle Power Consumption Diagnosis — `CANDIDATE`; [42] Energy Efficiency Scan — `CANDIDATE`; [55] Performance-per-Watt Benchmark — `IDEA`; [56] Thermal / Power Efficiency Comparison — `IDEA`; [57] Power-Limited Performance Detection — `CANDIDATE`; [58] Performance Regression Caused by Energy Policy — `IDEA`
...
Evita decisões baseadas em suposição sobre **Energy efficiency & performance-per-watt analysis**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Energy efficiency & performance-per-watt analysis; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Energy efficiency & performance-per-watt analysis** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-ENERGY-004
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Energy efficiency & performance-per-watt analysis** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/energy/c-energy-004/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Yes — controlled energy/performance-per-watt measurement with valid power source.
- Evidência realmente executada: No direct probe; `energy.active-scheme` is in a different/research feature and does not measure energy.
- Execução segura neste host / teste: On this desktop, OS state read is safe; meaningful package/system power measurement is not available. Teste recomendado: Measure identical bounded workload with authoritative power/energy telemetry, repeat and report uncertainty; explicitly mark unsupported without a sensor.
- Impedimento ou limitação restante: Desktop has no battery and no vendor power telemetry adapter; no cross-platform energy source is present.
