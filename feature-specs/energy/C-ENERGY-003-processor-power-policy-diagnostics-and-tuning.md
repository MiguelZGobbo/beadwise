# Feature Specification — C-ENERGY-003 — Processor power policy diagnostics & tuning

## 3. Identificação

Nome: Processor power policy diagnostics & tuning
ID: C-ENERGY-003
Tipo: Configuration / Optimization, Diagnostic, Diagnostic / Configuration, Optimization / Configuration, Optimization / Recommendation
...
**Base conceitual do Discovery:** [10] Processor Minimum Performance State — `CANDIDATE`; [11] Processor Maximum Performance State — `CANDIDATE`; [12] Processor Boost Mode Detection — `CANDIDATE`; [13] Processor Boost Policy Tuning — `RESEARCHING`; [14] Energy Performance Preference — EPP Detection — `CANDIDATE`; [15] EPP Tuning — `RESEARCHING`; [16] CPPC / Hardware Autonomous Performance Detection — `CANDIDATE`; [17] Core Parking State Detection — `CANDIDATE`; [18] Core Parking Policy Tuning — `RESEARCHING`; [19] Processor Idle State Diagnostics — `IDEA`; [53] Power Throttling State Analysis — `RESEARCHING`; [69] EcoQoS Capability Detection — `CANDIDATE`; [70] Process EcoQoS State Detection — `RESEARCHING`; [71] Background Work EcoQoS Advisor — `IDEA`; [72] Power Throttling Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Processor power policy diagnostics & tuning**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- active scheme and effective AC/DC settings through Power API/powercfg
- available sleep states, wake sources, wake timers and power requests through powercfg/documented Power APIs
- processor power policy through scheme values, not Registry myths
- energy/battery evidence through powercfg reports and battery APIs
- device power-management context correlated with device capability and platform type
- N/A
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Processor power policy diagnostics & tuning; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Processor power policy diagnostics & tuning**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
- powercfg /list /query /a /requests /lastwake /waketimers
- powercfg /energy /batteryreport /sleepstudy /systempowerreport
- GetSystemPowerStatus / battery device interfaces
...
```text
Feature: C-ENERGY-003
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Processor power policy diagnostics & tuning**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### APIs
PowerGetActiveScheme / PowerReadACValueIndex / PowerReadDCValueIndex, powercfg /list /query /a /requests /lastwake /waketimers, powercfg /energy /batteryreport /sleepstudy /systempowerreport, GetSystemPowerStatus / battery device interfaces

### Componentes do Windows
Componentes do subsistema ENERGY e infraestrutura comum de Event Log/ETW/CIM quando aplicável.
...
### Script/protótipo
Local: `/prototypes/energy/c-energy-003/` — **TBD / ainda não executado nesta fase documental**.
...
**Observed behavior (campanha 2026-09-08):** probes compartilhados read-only executados neste host: `energy.active-scheme`=PASS. Evidência: `/prototypes/system/C-SYSTEM-017/results/domain-evidence.json`. Estes sinais são parciais e não satisfazem, sozinhos, o gate DETECT completo desta feature.
...
- Microsoft recomenda que PPM customization siga orientação do silicon vendor; não há preset universal seguro.
- Detect pode expor settings/effective behavior, mas tuning só será permitido quando uma regra vendor/workload-specific possuir origem e medida.
- Não codificar min/max processor state, boost mode ou EPP magic values como otimização geral.
- `SPECIFIED` exige matriz AMD/Intel e mecanismo adaptativo ou recomendações oficialmente justificadas.
