# Feature Specification — C-ENERGY-012 — Energy Saver diagnostics & configuration

## 3. Identificação

Nome: Energy Saver diagnostics & configuration  
ID: C-ENERGY-012  
...
**Base conceitual do Discovery:** [51] Energy Saver State Detection — `CANDIDATE`; [52] Energy Saver Configuration — `CANDIDATE`; [96] Energy Saver Background Activity Impact — `CANDIDATE`; [97] Energy Saver Automatic Activation Policy — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Energy Saver diagnostics & configuration**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Energy Saver diagnostics & configuration; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Energy Saver diagnostics & configuration**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-ENERGY-012
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Energy Saver diagnostics & configuration**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/energy/c-energy-012/` — **TBD / ainda não executado nesta fase documental**.
...
- Windows 11 Energy Saver e versões anteriores Battery Saver não devem ser tratados como o mesmo contrato em todas as builds.
- Policy CSP não substitui necessariamente user preference consumer.
- Definir API pública/effective state por build, AC/DC behavior e rollback antes de Apply.
- Sem API suportada, manter diagnostics/deep-link apenas.
