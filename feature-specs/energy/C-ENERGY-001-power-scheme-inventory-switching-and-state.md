# Feature Specification — C-ENERGY-001 — Power scheme inventory, switching & state

## 3. Identificação

Nome: Power scheme inventory, switching & state
ID: C-ENERGY-001
Tipo: Configuration / Optimization, Diagnostic, Diagnostic / Configuration, Diagnostic / Product Behavior, Diagnostic / Repair, Monitoring, Safety / Configuration
...
**Base conceitual do Discovery:** [1] Power Scheme Inventory — `CANDIDATE`; [2] Power Scheme Switching — `CANDIDATE`; [3] Power Scheme Backup / Restore — `CANDIDATE`; [4] Power Scheme Configuration Diff — `IDEA`; [5] Broken / Anomalous Power Scheme Detection — `IDEA`; [6] Custom BeadWise Power Profile — `RESEARCHING`; [7] Ultimate Performance Availability — `CANDIDATE`; [61] User-Configured Power Mode Detection — `CANDIDATE`; [62] AC Power Mode Detection — `CANDIDATE`; [63] DC Power Mode Detection — `CANDIDATE`; [64] Effective Power Mode Detection — `CANDIDATE`; [65] Power Mode Change Monitoring — `IDEA`; [66] Power Scheme × Power Mode Interaction Analysis — `CANDIDATE`; [67] OEM Power Overlay Detection — `RESEARCHING`; [68] Effective Power Configuration Explanation — `IDEA`
...
Evita decisões baseadas em suposição sobre **Power scheme inventory, switching & state**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Power scheme inventory, switching & state; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Power scheme inventory, switching & state**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-ENERGY-001
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Power scheme inventory, switching & state**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/energy/c-energy-001/` — **TBD / ainda não executado nesta fase documental**.
