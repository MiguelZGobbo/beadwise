# Feature Specification — C-ENERGY-006 — Device power-management diagnostics

## 3. Identificação

Nome: Device power-management diagnostics  
ID: C-ENERGY-006  
Tipo: Configuration / Optimization, Diagnostic, Diagnostic / Repair  
...
**Base conceitual do Discovery:** [36] USB Selective Suspend State — `CANDIDATE`; [37] USB Selective Suspend Troubleshooter — `CANDIDATE`; [38] PCIe Link State Power Management Detection — `CANDIDATE`; [39] PCIe ASPM Policy Tuning — `RESEARCHING`
...
Evita decisões baseadas em suposição sobre **Device power-management diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Device power-management diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Device power-management diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-ENERGY-006
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Device power-management diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/energy/c-energy-006/` — **TBD / ainda não executado nesta fase documental**.
