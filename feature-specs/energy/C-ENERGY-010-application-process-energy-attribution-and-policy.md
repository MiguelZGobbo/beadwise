# Feature Specification — C-ENERGY-010 — Application/process energy attribution & policy

## 3. Identificação

Nome: Application/process energy attribution & policy  
ID: C-ENERGY-010  
Tipo: Configuration, Diagnostic, Diagnostic / Configuration, Optimization  
...
**Base conceitual do Discovery:** [45] Energy Consumption by Application — `IDEA`; [54] Per-Process Energy Efficiency Policy — `RESEARCHING`; [84] Hybrid Graphics Capability Detection — `CANDIDATE`; [85] Application GPU Power Preference — `CANDIDATE`; [86] Unnecessary dGPU Activation Detection — `CANDIDATE`; [87] GPU Power Preference Recommendation — `IDEA`
...
Evita decisões baseadas em suposição sobre **Application/process energy attribution & policy**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Application/process energy attribution & policy; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Application/process energy attribution & policy**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-ENERGY-010
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Application/process energy attribution & policy**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/energy/c-energy-010/` — **TBD / ainda não executado nesta fase documental**.
...
- Energy attribution pode usar OS telemetry, mas per-process efficiency/power throttling policy deve usar APIs públicas e respeitar process identity.
- Não punir processo apenas por consumo momentâneo; recommendation exige janela/contexto.
- Mutação de process priority/affinity pertence a Processes; Energy pode controlar somente power-related API suportada.
- Provar stale PID/session behavior.
