# Feature Specification — C-AUDIO-005 — Spatial, channel & speaker configuration

## 3. Identificação

Nome: Spatial, channel & speaker configuration  
ID: C-AUDIO-005  
Tipo: Diagnostic / Configuration, Tool / Diagnostic  
...
**Base conceitual do Discovery:** [17] Spatial Audio Capability Diagnostics — `CANDIDATE`; [18] Channel Layout & Speaker Configuration Diagnostics — `CANDIDATE`; [53] Speaker / Channel Test Tool — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Spatial, channel & speaker configuration**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Spatial, channel & speaker configuration; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Spatial, channel & speaker configuration**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Spatial, channel & speaker configuration**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/audio/c-audio-005/` — **TBD / ainda não executado nesta fase documental**.
...
- Spatial sound, channel layout e speaker configuration possuem APIs/capabilities diferentes; detecção não garante mutação genérica suportada.
- Não forçar stereo/5.1/7.1 ou spatial mode sem validar endpoint/content/use case.
- Definir fonte da verdade e Apply público para cada suboperação ou manter como diagnóstico/guia.
- Rollback deve restaurar configuração anterior exata, não um preset default.
