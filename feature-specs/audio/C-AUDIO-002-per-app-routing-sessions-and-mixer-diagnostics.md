# Feature Specification — C-AUDIO-002 — Per-app routing, sessions & mixer diagnostics
...
**Base conceitual do Discovery:** [5] Per-App Audio Routing Diagnostics — `CANDIDATE`; [6] Audio Session & Volume Mixer Diagnostics — `CANDIDATE`; [32] Communication Activity / Ducking Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Per-app routing, sessions & mixer diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Per-app routing, sessions & mixer diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Per-app routing, sessions & mixer diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Per-app routing, sessions & mixer diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/audio/c-audio-002/` — **TBD / ainda não executado nesta fase documental**.
...
- Enumerar sessões via `IAudioSessionManager2`/`IAudioSessionEnumerator`; volume/mute por sessão usa interfaces públicas quando a ação for explicitamente solicitada.
- “Per-app routing” persistente não deve ser inferido das sessões atuais; se não houver contrato suportado para alterar roteamento default por app, manter essa parte read-only/Unsupported.
- Snapshot para volume/mute captura valor original da sessão viva; sessão encerrada torna rollback stale.
- `PROVEN` exige sessão criada/encerrada e múltiplos apps.
