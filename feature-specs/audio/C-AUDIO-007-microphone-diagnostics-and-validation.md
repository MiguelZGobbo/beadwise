# Feature Specification — C-AUDIO-007 — Microphone diagnostics & validation
...
**Base conceitual do Discovery:** [23] Microphone Permission & Privacy Diagnostics — `CANDIDATE`; [24] Microphone Input Level Diagnostics — `CANDIDATE`; [45] Microphone Array & Beamforming Awareness — `CANDIDATE`; [54] Microphone Test & Clipping Validation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Microphone diagnostics & validation**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Microphone diagnostics & validation; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Microphone diagnostics & validation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Microphone diagnostics & validation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
- Detect combina capture endpoint availability, mute/volume quando suportado, permission/privacy state e captura controlada de nível/sinal.
- Não gravar/conservar áudio do usuário por padrão; teste deve ser efêmero e consentido.
- Privacy policy/restriction é `BLOCKED/MANAGED`, não algo a contornar.
- `PROVEN` exige mic saudável, muted/permission blocked e ausência de dispositivo.
