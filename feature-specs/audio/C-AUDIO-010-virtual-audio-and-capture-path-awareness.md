# Feature Specification — C-AUDIO-010 — Virtual audio & capture path awareness

## 3. Identificação

Nome: Virtual audio & capture path awareness
ID: C-AUDIO-010
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [33] Virtual Audio Device Awareness — `CANDIDATE`; [34] Audio Loopback / Capture Path Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Virtual audio & capture path awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- audio endpoints and defaults through MMDevice API
- per-app sessions/routing context through WASAPI audio sessions
- mix/device formats through IAudioClient GetMixFormat/IsFormatSupported
- device topology and endpoint properties through Core Audio APIs
- driver/device health through PnP and invalid-device errors
- glitch/latency evidence through ETW/WPR only when needed
- N/A
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Virtual audio & capture path awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Virtual audio & capture path awareness** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **Virtual audio & capture path awareness** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### APIs
IMMDeviceEnumerator / MMDevice API, WASAPI / IAudioClient, IAudioSessionManager2, DeviceTopology API, EndpointVolume API, ETW/WPR, SetupAPI

### Componentes do Windows
Componentes do subsistema AUDIO e infraestrutura comum de Event Log/ETW/CIM quando aplicável.
...
### Resultado
```text
DETECT: PARTIAL (Core Audio identity and PnP transport prefixes collected; virtual/physical and capture-path classification intentionally not inferred)
PLAN: NOT_TESTED
DRY-RUN: NOT_TESTED
APPLY: N/A
VERIFY: NOT_TESTED
...
**Prova segura adicional observada (2026-09-08):** Endpoint identity hashes/property-availability/states foram coletados por MMDevice. O runner também sumarizou PnP `AudioEndpoint`/`Media` por status/problem/enumerator/hash, com candidatos USB/Bluetooth/Root/virtual registrados sem IDs brutos. O prototype não converte nome em prova de virtual/physical ou loopback/capture path e mantém a correlação exata endpoint-to-parent como `NOT_TESTED`; o gate observado fica `PARTIAL`.

Evidência: /prototypes/audio/C-AUDIO-001/results/.
<!-- PHASE2-SAFE-PROOF-2026-09-08:END -->
<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Yes — actual virtual/physical endpoint and capture-path awareness.
- Evidência realmente executada: Prototype seguro adicional executado: Endpoint identity hashes/property-availability/states foram coletados por MMDevice. O runner também sumarizou PnP `AudioEndpoint`/`Media` por status/problem/enumerator/hash, com candidatos USB/Bluetooth/Root/virtual registrados sem IDs brutos. O prototype não converte nome em prova de virtual/physical ou loopback/capture path e mantém a correlação exata endpoint-to-parent como `NOT_TESTED`; o gate observado fica `PARTIAL`. Evidência: `/prototypes/audio/C-AUDIO-001/results/`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Yes, read-only endpoint/PnP inspection is safe. Teste recomendado: Enumerate virtual and physical render/capture endpoints, provider/parent path and direction; verify virtual loopback/capture is labeled and not treated as physical hardware.
