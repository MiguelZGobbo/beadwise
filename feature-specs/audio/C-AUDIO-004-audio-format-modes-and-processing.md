# Feature Specification — C-AUDIO-004 — Audio format, modes & processing

## 3. Identificação

Nome: Audio format, modes & processing  
ID: C-AUDIO-004  
...
**Base conceitual do Discovery:** [10] Audio Format Capability Inventory — `CANDIDATE`; [11] Default Audio Format Validation — `CANDIDATE`; [12] Shared Mode Audio Diagnostics — `CANDIDATE`; [13] Exclusive Mode Capability & Conflict Diagnostics — `CANDIDATE`; [14] Audio Enhancements Inventory — `CANDIDATE`; [15] Audio Enhancement Troubleshooting — `CANDIDATE`; [16] Audio Processing Object / APO Awareness — `CANDIDATE`; [25] Microphone Processing / Enhancement Diagnostics — `CANDIDATE`; [40] Low-Latency Shared Mode Diagnostics — `CANDIDATE`; [41] Raw Audio Processing Capability — `CANDIDATE`; [42] Audio Engine Period / Buffer Context — `CANDIDATE`; [43] Hardware Audio Offload Awareness — `CANDIDATE`; [46] Audio Processing Mode Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Audio format, modes & processing**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Audio format, modes & processing; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Audio format, modes & processing**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Audio format, modes & processing**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/audio/c-audio-004/` — **TBD / ainda não executado nesta fase documental**.
...
- Core Audio permite ler capabilities/formatos em várias superfícies, mas alterar o “Default Format” e enhancements/APO state do Windows não possui um único contrato público estável para todos os endpoints/drivers.
- Não escrever endpoint property store documentado como read-only.
- Antes de `SPECIFIED`, separar diagnóstico de formato suportado do Apply e definir API pública por operação/vendor quando existir.
- Não recomendar sample rate/bit depth universal como melhoria de qualidade/performance.
