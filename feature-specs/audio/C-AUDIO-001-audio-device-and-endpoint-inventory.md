# Feature Specification — C-AUDIO-001 — Audio device & endpoint inventory
...
**Base conceitual do Discovery:** [1] Audio Endpoint Inventory — `CANDIDATE`; [2] Physical Audio Device & Endpoint Relationship — `CANDIDATE`; [3] Default Playback Device Validation — `CANDIDATE`; [4] Default Capture Device Validation — `CANDIDATE`; [49] Audio Jack Detection & Connector State — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Audio device & endpoint inventory**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Audio device & endpoint inventory; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Audio device & endpoint inventory**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Audio device & endpoint inventory**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
- Inventário read-only via Core Audio (`IMMDeviceEnumerator`/endpoint properties) e PnP para contexto de dispositivo.
- Propriedades de endpoint definidas pelo sistema são leitura; não escrever property store como mecanismo de configuração.
- Diferenciar render/capture, active/disabled/unplugged/not-present e roles/default quando suportado.
- `PROVEN` exige múltiplos endpoints e hotplug.
