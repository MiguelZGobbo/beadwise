# Feature Specification — C-AUDIO-003 — Audio driver & device health
...
**Base conceitual do Discovery:** [7] Audio Device Health & Problem Code Diagnostics — `CANDIDATE`; [8] Audio Driver Inventory & Provider Diagnostics — `CANDIDATE`; [9] Audio Driver Regression Correlation — `CANDIDATE`; [28] Audio Engine / Service Health Diagnostics — `CANDIDATE`; [50] Vendor Audio Suite Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Audio driver & device health**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Audio driver & device health; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Audio driver & device health**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Audio driver & device health**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
- Diagnóstico via PnP problem status + endpoint state + eventos; ausência de endpoint não identifica sozinho falha de driver.
- Atualização/rollback de driver pertence ao domínio Drivers.
- Reparos de restart PnP devem ser delegados a workflow de repair e validados por reenumeração.
- `PROVEN` exige disabled/unplugged/problem-code e dispositivo saudável.
