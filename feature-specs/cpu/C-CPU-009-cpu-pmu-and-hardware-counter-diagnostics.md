# Feature Specification — C-CPU-009 — CPU PMU & hardware-counter diagnostics

## 3. Identificação

Nome: CPU PMU & hardware-counter diagnostics
ID: C-CPU-009
Tipo: Advanced Diagnostic, Advanced Diagnostic / Benchmark, Advanced Diagnostic Tool, Safety / Architecture, Shared Capability
...
**Base conceitual do Discovery:** [56] CPU Hardware Counter Profiling — `IDEA`; [57] IPC / Instructions-per-Cycle Analysis — `IDEA`; [58] Cache Miss Analysis — `IDEA`; [59] Branch Misprediction Analysis — `IDEA`; [120] PMU Capability Enumeration — `CANDIDATE`; [121] Instructions Retired Measurement — `CANDIDATE`; [122] CPU Cycle Measurement — `CANDIDATE`; [123] IPC Analysis — `CANDIDATE`; [124] PMU Sampling Profiling — `IDEA`; [125] PMU + Context Switch Correlation — `IDEA`; [126] Cache Performance Counter Analysis — `IDEA`; [127] Hardware Counter Portability Guard — `CANDIDATE`; [128] CPU Performance Source Selection — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **CPU PMU & hardware-counter diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for CPU PMU & hardware-counter diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **CPU PMU & hardware-counter diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **CPU PMU & hardware-counter diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/cpu/c-cpu-009/` — **TBD / ainda não executado nesta fase documental**.
...
- https://learn.microsoft.com/windows-server/administration/performance-tuning/in-depth-hardware-counter-profiling
- https://learn.microsoft.com/windows-hardware/test/wpt/windows-performance-recorder

<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Yes — PMU availability/semantics and validation against an independent profiler.
- Evidência realmente executada: No PMU/counter probe.
- Execução segura neste host / teste: A read-only availability query is safe, but raw PMU access may require a kernel driver/admin. Teste recomendado: Detect supported counters; sample a bounded known workload; compare instructions/cycles semantics with an independent profiler and return Unsupported when unavailable.
- Impedimento ou limitação restante: No profiler/vendor PMU adapter and no elevated counter path; architecture-specific proof is unavailable here.
...
- PMU é capability avançada e optional: enumerar/usar somente counters expostos por mecanismo suportado no hardware/OS; eventos são arquitetura/model-specific.
- IPC só é válido quando instructions-retired e cycle event semantics são compatíveis e amostrados no mesmo contexto.
- Não comparar raw event codes entre microarquiteturas.
- `PROVEN` exige validação contra profiler/vendor em pelo menos uma plataforma e `Unsupported` em plataforma sem acesso.
