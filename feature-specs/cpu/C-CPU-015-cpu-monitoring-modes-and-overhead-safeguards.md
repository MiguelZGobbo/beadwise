# Feature Specification — C-CPU-015 — CPU monitoring modes & overhead safeguards
...
**Base conceitual do Discovery:** [129] CPU Telemetry Sampling Cost Analysis — `IDEA`; [130] CPU Monitoring Low-Overhead Mode — `CANDIDATE`; [131] CPU Deep Diagnostic Mode — `IDEA`
...
Evita decisões baseadas em suposição sobre **CPU monitoring modes & overhead safeguards**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for CPU monitoring modes & overhead safeguards; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **CPU monitoring modes & overhead safeguards** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **CPU monitoring modes & overhead safeguards** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/cpu/c-cpu-015/` — **TBD / ainda não executado nesta fase documental**.
