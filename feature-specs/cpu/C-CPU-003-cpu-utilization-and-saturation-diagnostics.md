# Feature Specification — C-CPU-003 — CPU utilization & saturation diagnostics
...
**Base conceitual do Discovery:** [11] Current CPU Utilization — `CANDIDATE`; [12] Per-Core CPU Utilization — `CANDIDATE`; [13] CPU Utility vs CPU Time — `CANDIDATE`; [14] Single-Core Saturation Detection — `CANDIDATE`; [16] Processor Queue Analysis — `CANDIDATE`; [17] Context Switch Rate Monitoring — `CANDIDATE`; [18] User Time / Kernel Time Analysis — `CANDIDATE`; [19] Interrupt CPU Time Detection — `CANDIDATE`; [20] DPC CPU Time Detection — `CANDIDATE`; [62] CPU Background Interference Detection — `CANDIDATE`; [63] CPU Idle Baseline — `CANDIDATE`; [64] CPU Idle Anomaly Detection — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **CPU utilization & saturation diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for CPU utilization & saturation diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **CPU utilization & saturation diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **CPU utilization & saturation diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
