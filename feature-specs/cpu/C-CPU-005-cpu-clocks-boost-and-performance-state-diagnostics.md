# Feature Specification — C-CPU-005 — CPU clocks, boost & performance-state diagnostics
...
**Base conceitual do Discovery:** [21] CPU Clock Detection — `CANDIDATE`; [22] Per-Core Clock Monitoring — `CANDIDATE`; [23] Effective Clock Monitoring — `CANDIDATE`; [24] Base / Boost Frequency Validation — `CANDIDATE`; [25] Boost Behavior Analysis — `CANDIDATE`; [116] Clock Stretching Detection — `RESEARCHING`; [117] Reported Clock × Effective Clock Comparison — `CANDIDATE`; [118] Frequency Residency Analysis — `IDEA`; [119] CPU Performance State Transition Analysis — `IDEA`
...
Evita decisões baseadas em suposição sobre **CPU clocks, boost & performance-state diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **CPU clocks, boost & performance-state diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **CPU clocks, boost & performance-state diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/cpu/c-cpu-005/` — **TBD / ainda não executado nesta fase documental**.
