# Feature Specification — C-CPU-011 — CPU optimization myth & unsafe-tweak guards

## 3. Identificação

Nome: CPU optimization myth & unsafe-tweak guards
ID: C-CPU-011
Tipo: Safety, Safety / Product Behavior, Safety / Research
...
**Base conceitual do Discovery:** [76] CPU “Use All Cores” Myth Guard — `CANDIDATE`; [77] Scheduler Registry Tweak Guard — `CANDIDATE`; [79] Priority Tweak Myth Guard — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **CPU optimization myth & unsafe-tweak guards**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for CPU optimization myth & unsafe-tweak guards; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **CPU optimization myth & unsafe-tweak guards**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **CPU optimization myth & unsafe-tweak guards**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/cpu/c-cpu-011/` — **TBD / ainda não executado nesta fase documental**.
...
- https://learn.microsoft.com/windows/win32/procthread/scheduling-priorities
- https://learn.microsoft.com/windows-hardware/design/device-experiences/processor-power-management

<!-- PHASE2-SAFE-PROOF:START -->
**Prova prática segura executada (2026-09-08):** `C-CPU-011=PASS` em `/prototypes/cpu/C-CPU-001/results/cpu-safe-capabilities.json`. O gate negou os cinco candidatos inseguros/sem evidência (`use all cores`, registry pack de scheduler, realtime, affinity global e evidência unknown); o fingerprint de power scheme/estado observado permaneceu idêntico antes/depois e `mutationAttempted=false`. Nenhum tweak foi aplicado.
<!-- PHASE2-SAFE-PROOF:END -->

<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Yes — rule/fixture proof that unsafe or undocumented tweaks are rejected and no system mutation occurs.
- Evidência realmente executada: Prototype seguro adicional executado: `C-CPU-011=PASS` em `/prototypes/cpu/C-CPU-001/results/cpu-safe-capabilities.json`. O gate negou os cinco candidatos inseguros/sem evidência (`use all cores`, registry pack de scheduler, realtime, affinity global e evidência unknown); o fingerprint de power scheme/estado observado permaneceu idêntico antes/depois e `mutationAttempted=false`. Nenhum tweak foi aplicado. Evidência: `/prototypes/cpu/C-CPU-001/results/cpu-safe-capabilities.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Yes, pure fixture/rule and dry-run proof is safe; do not apply tweaks. Teste recomendado: Feed documented/undocumented/default-difference/tuning candidates into the safety gate; assert deny/Unknown, provenance and zero registry/power-plan changes.
...
- Safety gate documental: bloquear “use all cores” boot tweaks, scheduler Registry packs, forced realtime/high priority e affinity myths quando não há workload-specific evidence.
- Recomendação de CPU tuning exige mecanismo documentado, hipótese mensurável, hardware applicability e rollback.
- `Unknown evidence` nunca se converte em recomendação positiva.
- `PROVEN` exige testes de regra contra exemplos permitidos e bloqueados; Apply é N/A.
