# Feature Specification — C-GAMING-001 — Game discovery, identity & session context

## 3. Identificação

Nome: Game discovery, identity & session context
ID: C-GAMING-001
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [1] Game Library Discovery — `CANDIDATE`; [2] Game Process Identification — `CANDIDATE`; [3] Game Session Detection — `CANDIDATE`; [4] Game Runtime & Graphics API Detection — `CANDIDATE`; [5] Game Capability Profile — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Game discovery, identity & session context**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Game discovery, identity & session context; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Game discovery, identity & session context** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-GAMING-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Game discovery, identity & session context** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/gaming/c-gaming-001/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Yes — actual game discovery/identity and foreground/session context.
- Evidência realmente executada: `gaming.running-process-context` PASS only confirms a foreground process and process count; no game identity/session was present.
- Execução segura neste host / teste: Generic process inspection is safe; the essential game fixture is absent. Teste recomendado: Use a known installed/controlled game or launcher session; resolve executable/package/launcher identity, foreground ownership and absent/ambiguous state without launching unknown games.
- Impedimento ou limitação restante: No game session was running or validated; process presence alone cannot prove game discovery.
