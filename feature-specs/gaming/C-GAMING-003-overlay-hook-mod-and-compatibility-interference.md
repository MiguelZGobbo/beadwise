# Feature Specification — C-GAMING-003 — Overlay, hook, mod & compatibility interference

## 3. Identificação

Nome: Overlay, hook, mod & compatibility interference
ID: C-GAMING-003
...
**Base conceitual do Discovery:** [9] Overlay & Hook Interference Diagnostics — `CANDIDATE`; [31] Anti-Cheat & Game Security Compatibility Awareness — `CANDIDATE`; [52] Emulator & Compatibility-Layer Awareness — `CANDIDATE`; [53] Modded Game & Injection Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Overlay, hook, mod & compatibility interference**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Overlay, hook, mod & compatibility interference; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Overlay, hook, mod & compatibility interference** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-GAMING-003
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Overlay, hook, mod & compatibility interference** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/gaming/c-gaming-003/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Yes — overlay/hook/mod interference with an actual game and explicit non-causal reporting.
- Evidência realmente executada: No direct probe.
- Execução segura neste host / teste: Read-only module/process inspection is safe; injecting/removing overlays/mods is not. Teste recomendado: Inspect a controlled game process/module set and overlay configuration; compare clean vs known overlay fixture and return Unknown where ownership is unclear.
- Impedimento ou limitação restante: No game/overlay fixture; inducing hooks or disabling overlays on the live host is unsafe.
