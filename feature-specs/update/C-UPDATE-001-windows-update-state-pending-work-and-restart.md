# Feature Specification — C-UPDATE-001 — Windows Update state, pending work & restart

## 3. Identificação

Nome: Windows Update state, pending work & restart
ID: C-UPDATE-001
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [1] Windows Update State Inventory — `CANDIDATE`; [2] Pending Update Classification — `CANDIDATE`; [3] Pending Restart Detection — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Windows Update state, pending work & restart**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Windows Update state, pending work & restart; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Windows Update state, pending work & restart** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-UPDATE-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Windows Update state, pending work & restart** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/update/c-update-001/` — **TBD / ainda não executado nesta fase documental**.
...
- Limites preservados: no currently pending update/restart scenario
...
- Prova prática adicional essencial: Sim — state/pending/restart por fonte e inconsistência.
- Evidência realmente executada: Prototype seguro adicional executado: Update Agent history and pending-restart sources are reported independently. Resultado observado: `PASS`. Evidência: `/prototypes/update/C-UPDATE-001/results/update-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — Update Agent COM read-only search/state + detector Registry, preservando state atual.
- Impedimento ou limitação restante: Sem update pendente/restart real.
