# Feature Specification — C-UPDATE-004 — Update scheduling, pause & restart UX

## 3. Identificação

Nome: Update scheduling, pause & restart UX  
ID: C-UPDATE-004  
Tipo: Configuration, Diagnostic / Configuration  
...
**Base conceitual do Discovery:** [9] Update Deferral / Pause Awareness — `CANDIDATE`; [10] Active Hours & Restart UX — `CANDIDATE`; [11] Metered Connection Update Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Update scheduling, pause & restart UX**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Update scheduling, pause & restart UX; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Update scheduling, pause & restart UX**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-UPDATE-004
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Update scheduling, pause & restart UX**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/update/c-update-004/` — **TBD / ainda não executado nesta fase documental**.
