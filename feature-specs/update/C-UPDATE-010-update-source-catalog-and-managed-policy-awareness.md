# Feature Specification — C-UPDATE-010 — Update source/catalog & managed-policy awareness

## 3. Identificação

Nome: Update source/catalog & managed-policy awareness
ID: C-UPDATE-010
...
**Base conceitual do Discovery:** [19] Update Source / Catalog Fallback Awareness — `CANDIDATE`; [21] Managed Update Policy Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Update source/catalog & managed-policy awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- Windows Update service/search/history through supported Windows Update Agent or management surfaces
- pending reboot/install state from update result and documented servicing signals
- Delivery Optimization status/policy through documented cmdlets/policies
- update policy/managed state before interpreting pause, source or deferral
- lifecycle/support status from Microsoft release-health/lifecycle data, which is external and time-sensitive
- N/A
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Update source/catalog & managed-policy awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Update source/catalog & managed-policy awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
- Windows Update policy CSP / Group Policy
- Delivery Optimization PowerShell interfaces
- DISM package servicing for installed package context
- Microsoft lifecycle/release-health data
...
```text
Feature: C-UPDATE-010
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Update source/catalog & managed-policy awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/update/c-update-010/` — **TBD / ainda não executado nesta fase documental**.
...
- https://learn.microsoft.com/windows/deployment/update/waas-manage-updates-wsus
...
- Capability/premissa exercitada: Managed policy presence is reported separately from effective Update Agent behavior.
...
- Limites preservados: no WSUS/MDM managed host
...
- Prova prática adicional essencial: Sim — policy/source effective versus presence.
- Evidência realmente executada: Prototype seguro adicional executado: Managed policy presence is reported separately from effective Update Agent behavior. Resultado observado: `PARTIAL`. Evidência: `/prototypes/update/C-UPDATE-001/results/update-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — Registry/GPO/Update Agent read-only and fixtures `MANAGED/UNKNOWN`.
- Impedimento ou limitação restante: Sem WSUS/MDM/multi-source managed host.
...
- Read-only: identificar origem/gerenciamento quando observável (Windows Update/Microsoft Update/WSUS/policy) e não sobrescrever configuração administrada.
- Policy presence deve ser distinta de effective state; conflito/indeterminação retorna `MANAGED/UNKNOWN`.
- Não editar WSUS/MDM/GPO para “corrigir” máquina corporativa.
- `PROVEN` exige máquinas unmanaged e policy-managed.
