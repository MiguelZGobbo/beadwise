# Feature Specification — C-UPDATE-009 — Windows lifecycle & support status
...
**Base conceitual do Discovery:** [18] Windows Lifecycle / Support Status — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Windows lifecycle & support status**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Windows lifecycle & support status; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Windows lifecycle & support status** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-UPDATE-009
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Windows lifecycle & support status** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/update/c-update-009/` — **TBD / ainda não executado nesta fase documental**.
