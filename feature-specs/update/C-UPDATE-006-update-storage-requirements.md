# Feature Specification — C-UPDATE-006 — Update storage requirements

## 3. Identificação

Nome: Update storage requirements
ID: C-UPDATE-006
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [13] Update Storage Requirement Assessment — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Update storage requirements**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Update storage requirements; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Update storage requirements** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-UPDATE-006
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Update storage requirements** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/update/c-update-006/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: System-volume free space can be represented separately from an unknown update payload requirement.
...
- Limites preservados: no pending payload metadata; low-space case is fixture-only
...
- Prova prática adicional essencial: Sim — disk requirement/safe result.
- Evidência realmente executada: Prototype seguro adicional executado: System-volume free space can be represented separately from an unknown update payload requirement. Resultado observado: `PARTIAL`. Evidência: `/prototypes/update/C-UPDATE-001/results/update-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — volume/free-space calculation e Update Agent metadata where available; fixtures low-space.
- Impedimento ou limitação restante: Sem update payload/caso baixo espaço real.
