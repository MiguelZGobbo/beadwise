# Feature Specification — C-UPDATE-008 — Update rollback & regression correlation
...
**Base conceitual do Discovery:** [16] Update Uninstall / Rollback Availability — `CANDIDATE`; [17] Update Before/After Health Verification — `CANDIDATE`; [23] Update Regression Correlation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Update rollback & regression correlation**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Update rollback & regression correlation; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Update rollback & regression correlation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-UPDATE-008
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Update rollback & regression correlation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/update/c-update-008/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability diagnóstica: correlacionar timeline de updates com início de regressão sem afirmar causalidade apenas por proximidade temporal.
- Desinstalação/rollback de update não é executada por esta capability; delegar para mecanismo suportado e exibir reversibilidade/reboot do update específico.
- Confidence deve considerar evidência before/after, known issue oficial e reprodução.
- `PROVEN` exige fixtures com correlação verdadeira e falso positivo.
