# Feature Specification — C-HARDWARE-001 — Hardware inventory & firmware context
...
**Base conceitual do Discovery:** [1] Hardware Inventory Baseline — `CANDIDATE`; [13] Firmware / BIOS Context Inventory — `CANDIDATE`; [14] Hardware Sensor Availability Map — `CANDIDATE`; [17] Hardware Change Detection — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Hardware inventory & firmware context**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Hardware inventory & firmware context; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Hardware inventory & firmware context** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-HARDWARE-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Hardware inventory & firmware context** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/hardware/c-hardware-001/` — **TBD / ainda não executado nesta fase documental**.
