# Feature Specification — C-HARDWARE-009 — Hardware diagnostic session evidence bundle

## 3. Identificação

Nome: Hardware diagnostic session evidence bundle
ID: C-HARDWARE-009
Tipo: Diagnostic / Measurement, Tool / Diagnostic
...
**Base conceitual do Discovery:** [25] Hardware Diagnostic Session Baseline — `CANDIDATE`; [27] Hardware Diagnostic Evidence Bundle — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Hardware diagnostic session evidence bundle**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- PnP device inventory/topology through SetupAPI/CfgMgr32
- firmware/system identity through SMBIOS/CIM where documented
- hardware error evidence through WHEA records and Event Log/ETW
- cross-component correlation rather than declaring a fault from a single sensor
- vendor telemetry only as optional evidence with provenance
- N/A
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Hardware diagnostic session evidence bundle; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Hardware diagnostic session evidence bundle** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
- SetupDi* / CfgMgr32 CM_* APIs
- Win32_ComputerSystem / Win32_BIOS / SMBIOS interfaces
- Windows Event Log API
- WHEA error records/events
- ETW/WPR
...
```text
Feature: C-HARDWARE-009
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Hardware diagnostic session evidence bundle** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### APIs
SetupDi* / CfgMgr32 CM_* APIs, Win32_ComputerSystem / Win32_BIOS / SMBIOS interfaces, Windows Event Log API, WHEA error records/events, ETW/WPR

### Componentes do Windows
Componentes do subsistema HARDWARE e infraestrutura comum de Event Log/ETW/CIM quando aplicável.
...
### Script/protótipo
Local: `/prototypes/hardware/c-hardware-009/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: A hardware evidence package can hash direct identifiers and retain integrity without exporting raw values.
...
- Limites preservados: redaction proved with fixture; no failed hardware session
...
- Prova prática adicional essencial: Sim — pacote de sessão, minimização/redação e integridade.
- Evidência realmente executada: Prototype seguro adicional executado: A hardware evidence package can hash direct identifiers and retain integrity without exporting raw values. Resultado observado: `PASS`. Evidência: `/prototypes/hardware/C-HARDWARE-001/results/hardware-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — gerar pacote sintético+real mínimo, testar schema, hash e exclusão de PnP IDs/MAC.
- Impedimento ou limitação restante: Ainda não há sessão de hardware com falha nem matriz de privacidade.
