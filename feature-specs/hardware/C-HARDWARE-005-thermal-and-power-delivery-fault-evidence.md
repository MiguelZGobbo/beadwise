# Feature Specification — C-HARDWARE-005 — Thermal & power-delivery fault evidence

## 3. Identificação

Nome: Thermal & power-delivery fault evidence  
ID: C-HARDWARE-005  
Tipo: Diagnostic  
...
**Base conceitual do Discovery:** [15] Hardware Thermal Stress Evidence — `CANDIDATE`; [16] Power Delivery / PSU Symptom Correlation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Thermal & power-delivery fault evidence**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Thermal & power-delivery fault evidence; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Thermal & power-delivery fault evidence** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-HARDWARE-005
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Thermal & power-delivery fault evidence** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/hardware/c-hardware-005/` — **TBD / ainda não executado nesta fase documental**.
