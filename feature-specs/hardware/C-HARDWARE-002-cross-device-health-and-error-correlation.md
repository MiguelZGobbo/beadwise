# Feature Specification — C-HARDWARE-002 — Cross-device health & error correlation

## 3. Identificação

Nome: Cross-device health & error correlation
ID: C-HARDWARE-002
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [2] Device Manager Health Assessment — `CANDIDATE`; [3] Hardware Error Event Correlation — `CANDIDATE`; [4] Unexpected Shutdown / Hardware Failure Correlation — `CANDIDATE`; [24] Intermittent Device Failure Trend — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Cross-device health & error correlation**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Cross-device health & error correlation; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Cross-device health & error correlation** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-HARDWARE-002
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Cross-device health & error correlation** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/hardware/c-hardware-002/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — correlação de WHEA/PnP/EventLog sem inferir causalidade.
- Evidência realmente executada: NENHUMA.
- Execução segura neste host / teste: Parcial — consulta Event Log/WHEA e fixtures de timestamps/identidades.
- Impedimento ou limitação restante: Não há erro real conhecido para validar verdadeiro/falso positivo; critérios de correlação ainda genéricos.
