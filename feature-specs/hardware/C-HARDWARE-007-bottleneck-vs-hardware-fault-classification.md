# Feature Specification — C-HARDWARE-007 — Bottleneck vs hardware-fault classification

## 3. Identificação

Nome: Bottleneck vs hardware-fault classification  
ID: C-HARDWARE-007  
Tipo: Diagnostic  
...
**Base conceitual do Discovery:** [19] Cross-Component Bottleneck vs Fault Classification — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Bottleneck vs hardware-fault classification**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Bottleneck vs hardware-fault classification; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Bottleneck vs hardware-fault classification** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-HARDWARE-007
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Bottleneck vs hardware-fault classification** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/hardware/c-hardware-007/` — **TBD / ainda não executado nesta fase documental**.
...
- Bottleneck/fault classification é inferência multi-signal; precisa labeled scenarios/benchmarks, confidence and no single-metric causality.
