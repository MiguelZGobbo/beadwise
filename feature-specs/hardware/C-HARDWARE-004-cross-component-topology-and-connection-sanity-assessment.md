# Feature Specification — C-HARDWARE-004 — Cross-component topology & connection sanity assessment

## 3. Identificação

Nome: Cross-component topology & connection sanity assessment
ID: C-HARDWARE-004
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [9] PCIe Link Health & Error Awareness — `CANDIDATE`; [22] Memory Configuration Topology Sanity Check — `CANDIDATE`; [23] Storage Connection / Path Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Cross-component topology & connection sanity assessment**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Cross-component topology & connection sanity assessment; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Cross-component topology & connection sanity assessment** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-HARDWARE-004
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Cross-component topology & connection sanity assessment** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/hardware/c-hardware-004/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Cross-component topology can preserve per-source counts and explicit Unknown fields.
...
- Limites preservados: no dock, VM, laptop, or multi-controller matrix
...
- Prova prática adicional essencial: Sim — topologia cruzada e campos Unknown.
- Evidência realmente executada: Prototype seguro adicional executado: Cross-component topology can preserve per-source counts and explicit Unknown fields. Resultado observado: `PASS`. Evidência: `/prototypes/hardware/C-HARDWARE-001/results/hardware-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — correlacionar PnP, storage, display e SMBIOS neste host; validar IDs e `Unknown`.
- Impedimento ou limitação restante: Não cobre dock/VM/laptop/múltiplos controladores.
