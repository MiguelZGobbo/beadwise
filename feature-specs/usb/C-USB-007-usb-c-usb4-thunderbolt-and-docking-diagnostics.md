# Feature Specification — C-USB-007 — USB-C, USB4, Thunderbolt & docking diagnostics

## 3. Identificação

Nome: USB-C, USB4, Thunderbolt & docking diagnostics
ID: C-USB-007
Tipo: Diagnostic, Diagnostic / Repair Guidance
...
**Base conceitual do Discovery:** [23] USB-C Capability Diagnostics — `CANDIDATE`; [24] USB-C / Alternate Mode Failure Diagnostics — `CANDIDATE`; [25] USB4 Capability & Topology Diagnostics — `CANDIDATE`; [47] Thunderbolt / PCIe Tunneling Awareness — `CANDIDATE`; [49] Docking Station Composite Context Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **USB-C, USB4, Thunderbolt & docking diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for USB-C, USB4, Thunderbolt & docking diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **USB-C, USB4, Thunderbolt & docking diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **USB-C, USB4, Thunderbolt & docking diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-007/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — USB-C/USB4/Thunderbolt/dock path.
- Evidência realmente executada: E:usb.present-devices.pnp PASS (não prova essas classes).
- Execução segura neste host / teste: Sim só para `NotApplicable` detection.
- Impedimento ou limitação restante: Não há hardware USB-C/USB4/Thunderbolt/dock.
