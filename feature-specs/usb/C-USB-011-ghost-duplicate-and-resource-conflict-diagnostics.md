# Feature Specification — C-USB-011 — Ghost, duplicate & resource-conflict diagnostics

## 3. Identificação

Nome: Ghost, duplicate & resource-conflict diagnostics
ID: C-USB-011
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [30] Peripheral Duplicate / Ghost Device Diagnostics — `CANDIDATE`; [31] Peripheral Conflict & Resource Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Ghost, duplicate & resource-conflict diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Ghost, duplicate & resource-conflict diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Ghost, duplicate & resource-conflict diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **Ghost, duplicate & resource-conflict diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-011/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — ghost/duplicate/resource conflict classification.
- Evidência realmente executada: E:usb.present-devices.pnp PASS (sem problema).
- Execução segura neste host / teste: Parcial — PnP history/hidden devices read-only and synthetic fixtures.
- Impedimento ou limitação restante: Sem ghost/duplicate/conflito real; não criar device phantom.
