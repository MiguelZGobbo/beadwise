# Feature Specification — C-USB-006 — Peripheral driver, firmware & software-conflict diagnostics
...
**Base conceitual do Discovery:** [18] Peripheral Driver & Software Dependency Diagnostics — `CANDIDATE`; [19] Peripheral Firmware Awareness — `CANDIDATE`; [32] USB Driver Regression Correlation — `CANDIDATE`; [51] Peripheral Software Conflict Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Peripheral driver, firmware & software-conflict diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Peripheral driver, firmware & software-conflict diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Peripheral driver, firmware & software-conflict diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **Peripheral driver, firmware & software-conflict diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-006/` — **TBD / ainda não executado nesta fase documental**.
