# Feature Specification — C-USB-002 — USB speed, generation & bandwidth diagnostics

## 3. Identificação

Nome: USB speed, generation & bandwidth diagnostics
ID: C-USB-002
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [4] USB Connection Speed & Generation Validation — `CANDIDATE`; [20] USB Hub Capability & Load Diagnostics — `CANDIDATE`; [21] USB Bandwidth Contention Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **USB speed, generation & bandwidth diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- USB controllers/hubs/devices and descriptors through documented USB IOCTLs/SetupAPI
- negotiated connection information through USB hub IOCTLs where exposed
- PnP status and disconnect history through SetupAPI/Event Log/ETW
- power/suspend state interpreted with platform/device context
- USB4 topology/events through documented USB4 TraceLogging where available
- Bluetooth treated as separate radio/protocol context even when the controller is USB-attached
- N/A
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for USB speed, generation & bandwidth diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **USB speed, generation & bandwidth diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
- SetupAPI/CfgMgr32
- USB hub IOCTLs from USBIOCTL.H
- USBView sample approach as reference
- ETW USB providers
- USB4 TraceLogging providers
- powercfg/device power diagnostics
...
Target state: A normalized, provenance-aware assessment of **USB speed, generation & bandwidth diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### APIs
SetupAPI/CfgMgr32, USB hub IOCTLs from USBIOCTL.H, USBView sample approach as reference, ETW USB providers, USB4 TraceLogging providers, powercfg/device power diagnostics

### Componentes do Windows
Componentes do subsistema USB e infraestrutura comum de Event Log/ETW/CIM quando aplicável.
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-002/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Negotiated USB speed remains Unknown when documented properties do not expose it.
- Resultado observado deste probe: `UNSUPPORTED`.
- Evidência reproduzível: `/prototypes/usb/C-USB-001/results/usb-proof.json` (script e teste no mesmo prototype).
- Limites preservados: no hub IOCTL/USBView-equivalent negotiated-speed result on this host
...
- Prova prática adicional essencial: Sim — velocidade negociada precisa vir do caminho controller/hub/device real ou retornar Unsupported honestamente.
- Evidência realmente executada: Prototype seguro adicional executado: Negotiated USB speed remains Unknown when documented properties do not expose it. Resultado observado: `UNSUPPORTED`. Evidência: `/prototypes/usb/C-USB-001/results/usb-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: A enumeração read-only foi executada; não é seguro nem válido fabricar uma geração USB a partir de nome/PnP class.
- Impedimento ou limitação restante: API/descriptor e dispositivo USB 3/USB4 representativo indisponíveis neste ambiente bloqueiam a prova de velocidade.
