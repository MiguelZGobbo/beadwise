# Feature Specification — C-USB-005 — HID, mouse, keyboard, controller & input diagnostics
...
**Base conceitual do Discovery:** [13] HID Device Inventory — `CANDIDATE`; [14] Mouse & Keyboard Input Health Diagnostics — `CANDIDATE`; [15] HID Report Rate / Polling Diagnostics — `CANDIDATE`; [16] Input Latency Context Diagnostics — `CANDIDATE`; [17] Controller / Gamepad Diagnostics — `CANDIDATE`; [50] Virtual Controller & Input Remapping Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **HID, mouse, keyboard, controller & input diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for HID, mouse, keyboard, controller & input diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **HID, mouse, keyboard, controller & input diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **HID, mouse, keyboard, controller & input diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-005/` — **TBD / ainda não executado nesta fase documental**.
