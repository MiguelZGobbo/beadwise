# Feature Specification — C-USB-012 — Peripheral restart, repair & outcome validation
...
**Base conceitual do Discovery:** [33] Targeted Peripheral Restart / Re-enumeration — `CANDIDATE`; [34] Peripheral Repair Escalation — `CANDIDATE`; [55] Peripheral Repair Outcome Validation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Peripheral restart, repair & outcome validation**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Peripheral restart, repair & outcome validation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Peripheral restart, repair & outcome validation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-012/` — **TBD / ainda não executado nesta fase documental**.
...
- Apply genérico suportado fica limitado a restart/re-scan PnP quando dispositivo suporta e usuário autorizou (`pnputil /restart-device`, `/scan-devices`).
- Snapshot captura instance identity/problem state; restart não promete restaurar estado interno do periférico.
- Verify reenumera e compara health/presence; falha não dispara remove-device automaticamente.
- Driver rollback/update e power changes pertencem a outros owners.
