# Feature Specification — C-USB-001 — USB controller, topology & device inventory
...
**Base conceitual do Discovery:** [1] USB Controller Inventory — `CANDIDATE`; [2] USB Topology Mapping — `CANDIDATE`; [3] USB Device Identification — `CANDIDATE`; [39] Composite USB Device Diagnostics — `CANDIDATE`; [40] Multiple Identical Peripheral Identity Diagnostics — `CANDIDATE`; [41] Serial / COM Peripheral Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **USB controller, topology & device inventory**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for USB controller, topology & device inventory; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **USB controller, topology & device inventory**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **USB controller, topology & device inventory**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-001/` — **TBD / ainda não executado nesta fase documental**.
