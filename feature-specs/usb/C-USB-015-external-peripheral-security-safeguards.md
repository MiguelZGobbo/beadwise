# Feature Specification — C-USB-015 — External peripheral security safeguards

## 3. Identificação

Nome: External peripheral security safeguards
ID: C-USB-015
Tipo: Diagnostic / Safeguard, Safeguard, Safeguard / Diagnostic
...
**Base conceitual do Discovery:** [48] Kernel DMA Protection & External Peripheral Safeguard — `CANDIDATE`; [52] Accessibility Peripheral Safeguards — `CANDIDATE`; [53] Critical Input Device Repair Safeguard — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **External peripheral security safeguards**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **External peripheral security safeguards** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **External peripheral security safeguards** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-015/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Peripheral safeguard decisions preserve unknown and managed states without classifying by vendor name alone.
...
- Limites preservados: fixture rules; no malicious device or managed policy matrix
...
- Prova prática adicional essencial: Sim — security safeguard rule/source/Unknown.
- Evidência realmente executada: Prototype seguro adicional executado: Peripheral safeguard decisions preserve unknown and managed states without classifying by vendor name alone. Resultado observado: `PASS`. Evidência: `/prototypes/usb/C-USB-001/results/usb-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — PnP data plus fixtures for allow/block/manual review; no policy/device change.
- Impedimento ou limitação restante: Sem device malicious/managed policy matrix.
