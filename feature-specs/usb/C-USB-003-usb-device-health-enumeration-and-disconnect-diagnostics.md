# Feature Specification — C-USB-003 — USB device health, enumeration & disconnect diagnostics

## 3. Identificação

Nome: USB device health, enumeration & disconnect diagnostics
ID: C-USB-003
Tipo: Diagnostic, Diagnostic / Monitoring, Diagnostic / Repair
...
**Base conceitual do Discovery:** [5] USB Device Health & Problem Code Diagnostics — `CANDIDATE`; [6] USB Enumeration Failure Diagnostics — `CANDIDATE`; [7] USB Disconnect / Reconnect Monitoring — `CANDIDATE`; [8] USB Port / Hub Fault Correlation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **USB device health, enumeration & disconnect diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **USB device health, enumeration & disconnect diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **USB device health, enumeration & disconnect diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-003/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — disconnect/reconnect/error timeline.
- Evidência realmente executada: E:usb.present-devices.pnp PASS (estado presente saudável).
- Execução segura neste host / teste: Parcial — PnP/EventLog read-only e fixtures.
- Impedimento ou limitação restante: Sem dispositivo físico descartável para unplug/replug/erro; não fingir evento.
...
- Detect combina PnP problem status, presença/reenumeração e eventos correlacionados; uma desconexão isolada não prova cabo/porta/energia.
- Nenhum Apply genérico; reparo é delegado para restart/repair capability.
- Registrar timestamp e identidade estável quando possível para diferenciar reconnect de novo dispositivo.
- `PROVEN` exige unplug/replug, device error e hub path changes.
