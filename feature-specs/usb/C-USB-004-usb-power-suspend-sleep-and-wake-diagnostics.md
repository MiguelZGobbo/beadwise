# Feature Specification — C-USB-004 — USB power, suspend, sleep & wake diagnostics

## 3. Identificação

Nome: USB power, suspend, sleep & wake diagnostics  
ID: C-USB-004  
Tipo: Configuration / Repair, Diagnostic, Diagnostic / Configuration  
...
**Base conceitual do Discovery:** [9] USB Selective Suspend Diagnostics — `CANDIDATE`; [10] Device-Specific USB Power Exception — `CANDIDATE`; [11] USB Sleep / Resume Diagnostics — `CANDIDATE`; [12] USB Wake Capability Diagnostics — `CANDIDATE`; [22] USB Power Delivery / Power Availability Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **USB power, suspend, sleep & wake diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **USB power, suspend, sleep & wake diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **USB power, suspend, sleep & wake diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/usb/c-usb-004/` — **TBD / ainda não executado nesta fase documental**.
