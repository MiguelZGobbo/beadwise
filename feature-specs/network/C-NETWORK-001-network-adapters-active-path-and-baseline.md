# Feature Specification — C-NETWORK-001 — Network adapters, active path & baseline

## 3. Identificação

Nome: Network adapters, active path & baseline
ID: C-NETWORK-001
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [1] Network Adapter Inventory — `CANDIDATE`; [2] Active Network Path Identification — `CANDIDATE`; [3] Network Configuration Baseline — `CANDIDATE`; [47] Multi-Interface & Failover Diagnostics — `CANDIDATE`; [56] Docking, USB Ethernet & Adapter Transition Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Network adapters, active path & baseline**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Network adapters, active path & baseline; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Network adapters, active path & baseline** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-NETWORK-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Network adapters, active path & baseline** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-001/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Active adapter, IP configuration and default route can be correlated without exposing addresses.
...
- Limites preservados: no Wi-Fi/VPN transition
...
- Prova prática adicional essencial: Sim — path ativo, rota e adapter identity.
- Evidência realmente executada: Prototype seguro adicional executado: Active adapter, IP configuration and default route can be correlated without exposing addresses. Resultado observado: `PASS`. Evidência: `/prototypes/network/C-NETWORK-001/results/network-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — `Get-NetAdapter`, `Get-NetIPConfiguration`, `Get-NetRoute` e IP Helper snapshot, comparando a Ethernet Realtek ativa.
- Impedimento ou limitação restante: Sem Wi-Fi/VPN ativo e sem transição de rede.
