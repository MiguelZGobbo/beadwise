# Feature Specification — C-NETWORK-012 — Network power-management & sleep/wake correlation

## 3. Identificação

Nome: Network power-management & sleep/wake correlation  
ID: C-NETWORK-012  
...
**Base conceitual do Discovery:** [35] Network Adapter Power Management Diagnostics — `CANDIDATE`; [36] Energy Efficient Ethernet Diagnostics — `CANDIDATE`; [37] Wake-on-LAN Capability & Configuration — `CANDIDATE`; [68] Sleep / Wake Network Failure Correlation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Network power-management & sleep/wake correlation**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Network power-management & sleep/wake correlation; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Network power-management & sleep/wake correlation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-NETWORK-012
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Network power-management & sleep/wake correlation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-012/` — **TBD / ainda não executado nesta fase documental**.
...
- NIC power/EEE/wake mutation is device/vendor and overlaps Energy. Do not disable EEE/power saving universally; define delegated ownership.
