# Feature Specification — C-NETWORK-007 — Network driver/device health
...
**Base conceitual do Discovery:** [15] Network Driver & Device Health Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Network driver/device health**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Network driver/device health; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Network driver/device health** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-NETWORK-007
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Network driver/device health** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-007/` — **TBD / ainda não executado nesta fase documental**.
