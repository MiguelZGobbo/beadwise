# Feature Specification — C-NETWORK-011 — VPN, proxy, tunnel & virtual-network awareness

## 3. Identificação

Nome: VPN, proxy, tunnel & virtual-network awareness  
ID: C-NETWORK-011  
Tipo: Diagnostic, Diagnostic / Configuration, Diagnostic / Tool  
...
**Base conceitual do Discovery:** [31] VPN, Proxy & Tunnel Awareness — `CANDIDATE`; [32] Virtual Network Adapter & Virtual Switch Awareness — `CANDIDATE`; [60] Proxy / PAC / WPAD Diagnostics — `CANDIDATE`; [62] WSL & Hyper-V Network Architecture Awareness — `CANDIDATE`; [64] Native Packet Capture & Drop Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **VPN, proxy, tunnel & virtual-network awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for VPN, proxy, tunnel & virtual-network awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **VPN, proxy, tunnel & virtual-network awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-NETWORK-011
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **VPN, proxy, tunnel & virtual-network awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-011/` — **TBD / ainda não executado nesta fase documental**.
...
- VPN/proxy/virtual adapters are app/policy specific. Detect without disabling; repair requires owner/vendor and must preserve routes/DNS.
