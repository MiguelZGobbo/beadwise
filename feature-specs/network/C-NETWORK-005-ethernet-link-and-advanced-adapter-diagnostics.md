# Feature Specification — C-NETWORK-005 — Ethernet link & advanced adapter diagnostics

## 3. Identificação

Nome: Ethernet link & advanced adapter diagnostics
ID: C-NETWORK-005
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [10] Ethernet Link Diagnostics — `CANDIDATE`; [11] Ethernet Negotiation & Duplex Anomaly Detection — `CANDIDATE`; [16] Network Adapter Advanced Property Inventory — `CANDIDATE`; [63] Layer-2 Advanced Configuration Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Ethernet link & advanced adapter diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Ethernet link & advanced adapter diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Ethernet link & advanced adapter diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-NETWORK-005
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Ethernet link & advanced adapter diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-005/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Ethernet advanced properties can be read without invoking a setter.
...
- Limites preservados: no cable fault or alternate negotiation
...
- Prova prática adicional essencial: Sim — link Ethernet/propriedades avançadas e `AccessDenied`.
- Evidência realmente executada: Prototype seguro adicional executado: Ethernet advanced properties can be read without invoking a setter. Resultado observado: `PASS`. Evidência: `/prototypes/network/C-NETWORK-001/results/network-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — consultar `Get-NetAdapterAdvancedProperty`/`Get-NetAdapterHardwareInfo` na Realtek, sem Set.
- Impedimento ou limitação restante: Não testa negociação diferente, cabo defeituoso nem setter/admin.
