# Feature Specification — C-NETWORK-002 — Connectivity, reachability & route diagnostics

## 3. Identificação

Nome: Connectivity, reachability & route diagnostics
ID: C-NETWORK-002
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [4] End-to-End Connectivity Diagnostics — `CANDIDATE`; [6] Route & Hop Diagnostics — `CANDIDATE`; [48] Captive Portal / Limited Connectivity Detection — `CANDIDATE`; [66] Firewall / Port Reachability Diagnostics — `CANDIDATE`; [67] Local Network Service Reachability Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Connectivity, reachability & route diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Connectivity, reachability & route diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Connectivity, reachability & route diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-NETWORK-002
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Connectivity, reachability & route diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-002/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Loopback and current gateway reachability return separate structured outcomes.
...
- Limites preservados: no external target or captive portal
...
- Prova prática adicional essencial: Sim — reachability/rota e falha estruturada.
- Evidência realmente executada: Prototype seguro adicional executado: Loopback and current gateway reachability return separate structured outcomes. Resultado observado: `PASS`. Evidência: `/prototypes/network/C-NETWORK-001/results/network-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — `Test-NetConnection` para loopback e gateway/rota atual, preservando timeout/DNS failure.
- Impedimento ou limitação restante: Resultado externo depende da rede; não prova captive portal.
