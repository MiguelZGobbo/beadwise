# Feature Specification — C-NETWORK-014 — Staged network repair & rollback

## 3. Identificação

Nome: Staged network repair & rollback  
ID: C-NETWORK-014  
Tipo: Diagnostic / Benchmark, Diagnostic / Repair, Repair, Repair / Safeguard, Safeguard / Repair  
...
**Base conceitual do Discovery:** [40] Staged Network Repair — `CANDIDATE`; [41] DHCP Lease & Address Repair — `CANDIDATE`; [42] DNS Cache & Resolver Repair — `CANDIDATE`; [43] Winsock / TCP-IP Stack Repair — `CANDIDATE`; [44] Full Network Reset Safeguard — `CANDIDATE`; [50] Network Change Backup & Rollback — `CANDIDATE`; [51] Network Optimization Outcome Validation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Staged network repair & rollback**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Staged network repair & rollback; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Staged network repair & rollback**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-NETWORK-014
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Staged network repair & rollback**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-014/` — **TBD / ainda não executado nesta fase documental**.
...
- Network reset commands have different destructiveness and may lose static IP/VPN/proxy config. Need exact snapshot/reapply validation for every stage; avoid universal winsock/IP reset.
