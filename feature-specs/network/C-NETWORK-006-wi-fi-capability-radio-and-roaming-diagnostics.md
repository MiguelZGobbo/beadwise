# Feature Specification — C-NETWORK-006 — Wi-Fi capability, radio & roaming diagnostics
...
**Base conceitual do Discovery:** [12] Wi-Fi Capability & Connection Diagnostics — `CANDIDATE`; [13] Wi-Fi Signal & Radio Environment Diagnostics — `CANDIDATE`; [14] Wi-Fi Connection History & WLAN Report — `CANDIDATE`; [52] Wi-Fi Generation & Multi-Link Capability Diagnostics — `CANDIDATE`; [53] Wi-Fi Roaming & BSSID Transition Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Wi-Fi capability, radio & roaming diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Wi-Fi capability, radio & roaming diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Wi-Fi capability, radio & roaming diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-NETWORK-006
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Wi-Fi capability, radio & roaming diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-006/` — **TBD / ainda não executado nesta fase documental**.
