# Feature Specification — C-NETWORK-004 — DNS & name-resolution diagnostics

## 3. Identificação

Nome: DNS & name-resolution diagnostics
ID: C-NETWORK-004
Tipo: Benchmark, Diagnostic, Diagnostic / Configuration
...
**Base conceitual do Discovery:** [7] DNS Configuration & Resolution Diagnostics — `CANDIDATE`; [8] Encrypted DNS / DoH Awareness — `CANDIDATE`; [9] DNS Resolver Performance & Reliability Comparison — `CANDIDATE`; [61] Split DNS & Name Resolution Policy Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **DNS & name-resolution diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for DNS & name-resolution diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **DNS & name-resolution diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-NETWORK-004
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **DNS & name-resolution diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-004/` — **TBD / ainda não executado nesta fase documental**.
...
**Observed behavior (campanha 2026-09-08):** probes compartilhados read-only executados neste host: `network.dns-localhost`=PASS. Evidência: `/prototypes/system/C-SYSTEM-017/results/domain-evidence.json`. Estes sinais são parciais e não satisfazem, sozinhos, o gate DETECT completo desta feature.
...
- Detect resolver/config/query behavior, but DNS Apply must preserve per-interface/DHCP policy and not auto-switch provider; rollback exact original including automatic state.
