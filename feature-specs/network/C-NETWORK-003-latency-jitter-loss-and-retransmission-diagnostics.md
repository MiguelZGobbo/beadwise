# Feature Specification — C-NETWORK-003 — Latency, jitter, loss & retransmission diagnostics

## 3. Identificação

Nome: Latency, jitter, loss & retransmission diagnostics
ID: C-NETWORK-003
...
**Base conceitual do Discovery:** [5] Latency, Jitter & Packet Loss Diagnostics — `CANDIDATE`; [58] TCP Retransmission & Loss-Recovery Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Latency, jitter, loss & retransmission diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- adapter and IP configuration through IP Helper/NetAdapter surfaces
- route and active-path information through IP Helper routing tables
- DNS resolution through DnsQueryEx/system resolver behavior
- TCP connection/retransmission and interface counters through IP Helper/Performance Counters/ETW
- Wi-Fi state through Native Wi-Fi API
- firewall/profile state through NetSecurity/WFP-aware interfaces
- Use time-series/ETW evidence; fixed thresholds are not universal unless grounded in scenario-specific methodology.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Latency, jitter, loss & retransmission diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Latency, jitter, loss & retransmission diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
- GetAdaptersAddresses / GetIfTable2 / GetIfEntry2
- GetIpForwardTable2
- DnsQueryEx
- IcmpSendEcho2
- Native Wi-Fi API (Wlan*)
- Get-NetTCPConnection / NetTCPIP
- Windows Filtering Platform / NetSecurity
...
```text
Feature: C-NETWORK-003
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Latency, jitter, loss & retransmission diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/network/c-network-003/` — **TBD / ainda não executado nesta fase documental**.
...
**Observed behavior (campanha 2026-09-08):** probes compartilhados read-only executados neste host: `network.loopback-reachability`=PASS. Evidência: `/prototypes/system/C-SYSTEM-017/results/domain-evidence.json`. Estes sinais são parciais e não satisfazem, sozinhos, o gate DETECT completo desta feature.

<!-- PHASE2-SAFE-PROOF-20260908:START -->
**Observed safe proof (auditoria final, 2026-09-08):**
- Capability/premissa exercitada: A temporal sample reports jitter separately from TCP statistics and does not pretend loopback is WAN.
...
- Limites preservados: no controlled WAN target or impairment
...
- Prova prática adicional essencial: Sim — jitter/loss/retransmission por janela, não ping isolado.
- Evidência realmente executada: Prototype seguro adicional executado: A temporal sample reports jitter separately from TCP statistics and does not pretend loopback is WAN. Resultado observado: `PARTIAL`. Evidência: `/prototypes/network/C-NETWORK-001/results/network-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Parcial — 30 amostras loopback + `Get-NetTCPStatistics`; registrar que não representa WAN.
- Impedimento ou limitação restante: Sem alvo externo controlado/impairment para perda e jitter reais.
