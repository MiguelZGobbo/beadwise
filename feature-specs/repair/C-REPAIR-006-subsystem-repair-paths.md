# Feature Specification — C-REPAIR-006 — Subsystem repair paths
...
**Base conceitual do Discovery:** [11] Network Stack Repair Triage — `CANDIDATE`; [12] Audio Device Repair Triage — `CANDIDATE`; [15] File System Error Repair Triage — `CANDIDATE`; [16] Time & Clock Repair Triage — `CANDIDATE`; [17] Search / Index Repair Triage — `CANDIDATE`; [18] Printing Repair Triage — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Subsystem repair paths**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Subsystem repair paths; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Subsystem repair paths**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-REPAIR-006
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Subsystem repair paths**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/repair/c-repair-006/` — **TBD / ainda não executado nesta fase documental**.
