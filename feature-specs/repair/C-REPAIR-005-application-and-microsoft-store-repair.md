# Feature Specification — C-REPAIR-005 — Application & Microsoft Store repair
...
**Base conceitual do Discovery:** [9] Application Repair / Reset Awareness — `CANDIDATE`; [10] Microsoft Store / Packaged App Repair — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Application & Microsoft Store repair**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Application & Microsoft Store repair; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Application & Microsoft Store repair**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-REPAIR-005
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Application & Microsoft Store repair**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/repair/c-repair-005/` — **TBD / ainda não executado nesta fase documental**.
