# Feature Specification — C-REPAIR-004 — Pending operation & restart resolution
...
**Base conceitual do Discovery:** [7] Restart / Pending Operation Resolution — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Pending operation & restart resolution**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Pending operation & restart resolution; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Pending operation & restart resolution**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-REPAIR-004
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Pending operation & restart resolution**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/repair/c-repair-004/` — **TBD / ainda não executado nesta fase documental**.
