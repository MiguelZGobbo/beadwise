# Feature Specification — C-REPAIR-007 — Device & driver repair path

## 3. Identificação

Nome: Device & driver repair path
ID: C-REPAIR-007
Tipo: Repair
...
**Base conceitual do Discovery:** [13] Device Re-enumeration / Restart Guidance — `CANDIDATE`; [14] Driver Rollback / Reinstall Repair Path — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Device & driver repair path**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Device & driver repair path; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Device & driver repair path**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-REPAIR-007
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Device & driver repair path**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/repair/c-repair-007/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — PnP restart/re-scan com Verify.
- Evidência realmente executada: NENHUMA.
- Execução segura neste host / teste: Não integralmente — apenas inventário/`pnputil` help/dry-run.
- Impedimento ou limitação restante: Restart de dispositivo real pode cortar rede/entrada; não há dispositivo sacrificável.
