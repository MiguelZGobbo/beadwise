# Feature Specification — C-REPAIR-001 — System repair triage & orchestration

## 3. Identificação

Nome: System repair triage & orchestration
ID: C-REPAIR-001
Tipo: Diagnostic, Repair / Diagnostic, Tool / Diagnostic
...
**Base conceitual do Discovery:** [1] System Repair Triage — `CANDIDATE`; [8] Built-in Troubleshooter Orchestration — `CANDIDATE`; [29] Repair Session Bundling — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **System repair triage & orchestration**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for System repair triage & orchestration; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **System repair triage & orchestration**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-REPAIR-001
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **System repair triage & orchestration**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/repair/c-repair-001/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — roteamento e propagação de erro/admin.
- Evidência realmente executada: E:repair.component-store-check FAIL `DISM_EXIT_740_ADMIN_REQUIRED`.
- Execução segura neste host / teste: Sim para fixtures de roteamento e para preservar/interpretar 740; não Apply.
- Impedimento ou limitação restante: Diagnóstico completo/reparador delegado requer sessão admin e cenários reais.
...
- A capacidade é um orquestrador: não inventa mutações próprias; classifica sintomas/evidências e delega para o reparador owner (component store, AppX, PnP, boot etc.).
- ChangePlan deve listar apenas operações suportadas do reparador delegado e seus requisitos de admin/reboot/reversibilidade.
- Resultado `NO_SUPPORTED_REPAIR_PATH` é válido; não executar “repair packs”, Registry cleanup ou comandos não relacionados por tentativa e erro.
- `PROVEN` permanece pendente até exercitar a matriz de roteamento e falhas em Windows real.
