# Feature Specification — C-REPAIR-002 — System file & component-store repair
...
**Base conceitual do Discovery:** [2] System File Integrity Assessment — `CANDIDATE`; [3] Component Store Health & Repair — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **System file & component-store repair**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for System file & component-store repair; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **System file & component-store repair**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-REPAIR-002
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **System file & component-store repair**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/repair/c-repair-002/` — **TBD / ainda não executado nesta fase documental**.
...
- Método suportado definido: diagnosticar component store com DISM (`/CheckHealth`/`/ScanHealth` quando aplicável), reparar com `/RestoreHealth` quando necessário e executar `sfc /scannow` para arquivos protegidos.
- Verificação deve repetir o diagnóstico apropriado e interpretar o resultado/registro, não apenas exit code.
- Reparos substituem conteúdo corrompido por conteúdo considerado íntegro pelo servicing stack; não existe rollback específico confiável para “recolocar o arquivo corrompido”, portanto rollback = N/A e isso deve ser explícito.
- Fonte alternativa do DISM só pode ser usada quando compatível com versão/edição/build; não assumir imagem aleatória como válida.
- `PROVEN` exige corrupção/controlado ou ambiente de teste que demonstre detectar → reparar → verificar.
