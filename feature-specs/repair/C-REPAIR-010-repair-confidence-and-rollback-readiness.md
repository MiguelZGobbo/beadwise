# Feature Specification — C-REPAIR-010 — Repair confidence & rollback readiness

## 3. Identificação

Nome: Repair confidence & rollback readiness
ID: C-REPAIR-010
Tipo: Recommendation / Diagnostic, Safety / Diagnostic
...
**Base conceitual do Discovery:** [27] Repair Confidence & Evidence Summary — `CANDIDATE`; [28] Repair Rollback Readiness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Repair confidence & rollback readiness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- Capture exact pre-change state and existence/provenance; rollback restores that state, never an assumed default.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Repair confidence & rollback readiness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Repair confidence & rollback readiness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-REPAIR-010
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Repair confidence & rollback readiness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/repair/c-repair-010/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Admin, snapshot and reboot gates degrade rollback readiness to FULL/PARTIAL/NO explicitly.
...
- Limites preservados: fixture gates; no real repair integration
...
- Prova prática adicional essencial: Sim — gates Fully/Partially/No reversible e snapshot failure.
- Evidência realmente executada: Prototype seguro adicional executado: Admin, snapshot and reboot gates degrade rollback readiness to FULL/PARTIAL/NO explicitly. Resultado observado: `PASS`. Evidência: `/prototypes/repair/C-REPAIR-006/results/repair-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — fixtures de readiness/snapshot/reboot/admin, incluindo `UNKNOWN` degradando confidence.
- Impedimento ou limitação restante: Integração com reparadores reais continua bloqueada.
