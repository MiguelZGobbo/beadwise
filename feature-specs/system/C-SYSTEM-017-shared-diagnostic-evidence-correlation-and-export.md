# Feature Specification — C-SYSTEM-017 — Shared diagnostic evidence correlation & export
...
**Base conceitual do Discovery:** [44] System Diagnostic Export — `IDEA`; [45] Diagnostic Evidence Correlation — `IDEA`; [70] System Diagnostic Capture Package — `IDEA`
...
Evita decisões baseadas em suposição sobre **Shared diagnostic evidence correlation & export**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Shared diagnostic evidence correlation & export; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Shared diagnostic evidence correlation & export** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-SYSTEM-017
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Shared diagnostic evidence correlation & export** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
