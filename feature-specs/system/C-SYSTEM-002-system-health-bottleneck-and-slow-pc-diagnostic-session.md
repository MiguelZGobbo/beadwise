# Feature Specification — C-SYSTEM-002 — System health, bottleneck & slow-PC diagnostic session

## 3. Identificação

Nome: System health, bottleneck & slow-PC diagnostic session
ID: C-SYSTEM-002
Tipo: Diagnostic, Diagnostic Tool, Product Behavior, Shared Capability
...
**Base conceitual do Discovery:** [4] System Health Scan — `CANDIDATE`; [6] System Bottleneck Overview — `IDEA`; [31] “Why Is My PC Slow?” Diagnostic Session — `IDEA`; [48] System Diagnostic Framework — `CANDIDATE`; [49] “Nothing To Optimize” Result — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **System health, bottleneck & slow-PC diagnostic session**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for System health, bottleneck & slow-PC diagnostic session; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **System health, bottleneck & slow-PC diagnostic session** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-SYSTEM-002
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **System health, bottleneck & slow-PC diagnostic session** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/system/c-system-002/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — health/bottleneck session e fonte/weighting de correlação.
- Evidência realmente executada: NENHUMA.
- Execução segura neste host / teste: Parcial — executar coleta das entradas existentes e fixtures de combinação/Unknown.
- Impedimento ou limitação restante: “slow PC”/bottleneck não tem ground truth e regra/owner ainda genéricos.
