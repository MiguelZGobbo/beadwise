# Feature Specification — C-SYSTEM-018 — File/resource lock diagnosis & cleanup

## 3. Identificação

Nome: File/resource lock diagnosis & cleanup
ID: C-SYSTEM-018
Tipo: Diagnostic Tool, Repair / Tool
...
**Base conceitual do Discovery:** [53] Open File / Resource Lock Diagnosis — `CANDIDATE`; [54] Locked File Cleanup Assistant — `IDEA`
...
Evita decisões baseadas em suposição sobre **File/resource lock diagnosis & cleanup**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for File/resource lock diagnosis & cleanup; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **File/resource lock diagnosis & cleanup**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SYSTEM-018
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **File/resource lock diagnosis & cleanup**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/system/c-system-018/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability/premissa exercitada: Restart Manager identifies the owner of a prototype-owned file lock; cleanup and restart target only that child process.
- Resultado observado deste probe: `PASS`.
- Evidência reproduzível: `/prototypes/system/C-SYSTEM-018/results/restart-manager-proof.json` (script e teste no mesmo prototype).
- Limites preservados: does not exercise services, critical processes, kernel or antivirus locks; RmShutdown/RmRestart intentionally not invoked
...
- Prova prática adicional essencial: Sim — Restart Manager lock ownership, consent and safe cleanup.
- Evidência realmente executada: Prototype seguro adicional executado: Restart Manager identifies the owner of a prototype-owned file lock; cleanup and restart target only that child process. Resultado observado: `PASS`. Evidência: `/prototypes/system/C-SYSTEM-018/results/restart-manager-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim, serial — arquivo temp + processo filho próprio que mantém lock; `RmRegisterResources/RmGetList`, terminar somente filho, Verify/restart.
- Impedimento ou limitação restante: Não testar process/service critical ou lock kernel/antivírus.
...
- Diagnóstico de arquivos em uso deve preferir Restart Manager para registrar recursos e identificar aplicações/serviços relacionados quando suportado.
- “Cleanup” não significa desbloqueio forçado: oferecer fechar/reiniciar apenas quando suportado e com consentimento; processos críticos/serviços protegidos são bloqueados.
- Se o lock não puder ser atribuído por API suportada, retornar `UNKNOWN_LOCK_OWNER`; não usar driver/kernel tricks.
- Terminar processo não é rollback-reversível e deve ser operação destrutiva separada.
