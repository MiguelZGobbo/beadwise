# Feature Specification — C-SYSTEM-001 — System identity, baseline & compatibility context
...
**Base conceitual do Discovery:** [1] System Baseline / Estado Inicial do Sistema — `CANDIDATE`; [2] Windows Identification — `CANDIDATE`; [3] System Capability Detection — `CANDIDATE`; [47] System Defaults Reference — `RESEARCHING`
...
Evita decisões baseadas em suposição sobre **System identity, baseline & compatibility context**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for System identity, baseline & compatibility context; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **System identity, baseline & compatibility context** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-SYSTEM-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **System identity, baseline & compatibility context** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
