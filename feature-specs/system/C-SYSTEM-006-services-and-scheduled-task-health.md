# Feature Specification — C-SYSTEM-006 — Services & scheduled-task health

## 3. Identificação

Nome: Services & scheduled-task health  
ID: C-SYSTEM-006  
Tipo: Diagnostic, Diagnostic / Configuration, Diagnostic / Repair  
...
**Base conceitual do Discovery:** [12] Windows Services Inventory — `CANDIDATE`; [13] Service Dependency Analysis — `CANDIDATE`; [14] Service Failure Detection — `CANDIDATE`; [15] Scheduled Tasks Inventory — `CANDIDATE`; [16] Scheduled Task Failure Detection — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Services & scheduled-task health**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Services & scheduled-task health; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Services & scheduled-task health**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SYSTEM-006
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Services & scheduled-task health**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/system/c-system-006/` — **TBD / ainda não executado nesta fase documental**.
...
- https://learn.microsoft.com/windows/win32/taskschd/task-scheduler-1-0-interfaces
...
- Inventário/health é documentável via Service Control Manager e Task Scheduler, porém Configuration/Repair genérico continua amplo demais.
- Não criar lista universal de “serviços inúteis” nem desabilitar scheduled tasks por nome sem ownership/evidência.
- Antes de `SPECIFIED`, separar diagnóstico read-only das operações mutáveis específicas e definir snapshot de startup type/task enabled state + dependências.
- Services/tasks controlados por policy/vendor devem permanecer `MANAGED/UNSUPPORTED` para Apply.
