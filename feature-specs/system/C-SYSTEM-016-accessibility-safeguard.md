# Feature Specification — C-SYSTEM-016 — Accessibility safeguard
...
**Base conceitual do Discovery:** [43] Accessibility Configuration Safeguard — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Accessibility safeguard**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Accessibility safeguard; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Accessibility safeguard** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-SYSTEM-016
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Accessibility safeguard** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/system/c-system-016/` — **TBD / ainda não executado nesta fase documental**.
