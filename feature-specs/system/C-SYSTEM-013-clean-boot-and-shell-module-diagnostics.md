# Feature Specification — C-SYSTEM-013 — Clean-boot & shell/module diagnostics
...
**Base conceitual do Discovery:** [32] Clean Boot Diagnostic Assistant — `IDEA`; [39] Explorer / Shell Extension Diagnosis — `IDEA`; [55] Loaded Module Inventory — `CANDIDATE`; [56] Unsigned / Untrusted Module Detection — `IDEA`; [57] Executable Trust Metadata — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Clean-boot & shell/module diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Clean-boot & shell/module diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Clean-boot & shell/module diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-SYSTEM-013
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Clean-boot & shell/module diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/system/c-system-013/` — **TBD / ainda não executado nesta fase documental**.
