# Feature Specification — C-SYSTEM-008 — Windows feature & core-component health

## 3. Identificação

Nome: Windows feature & core-component health  
ID: C-SYSTEM-008  
...
**Base conceitual do Discovery:** [18] Optional Windows Features Inventory — `CANDIDATE`; [19] Windows Feature Dependency Safeguard — `IDEA`; [40] Windows Core Component Availability Check — `IDEA`; [69] Environment Configuration Health — `IDEA`
...
Evita decisões baseadas em suposição sobre **Windows feature & core-component health**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Windows feature & core-component health; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Windows feature & core-component health**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SYSTEM-008
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Windows feature & core-component health**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/system/c-system-008/` — **TBD / ainda não executado nesta fase documental**.
...
- https://learn.microsoft.com/windows-hardware/manufacture/desktop/enable-or-disable-windows-features-using-dism
...
- DISM fornece inventário e enable/disable de Optional Features, mas “core component health” não deve ser tratado como o mesmo mecanismo.
- Antes de mutar Optional Features, modelar dependências, payload/source availability, reboot e rollback para o estado original.
- Não desabilitar features do Windows por suposto ganho de performance sem benefício específico.
- Component-store repair pertence ao domínio Repair; evitar duplicação de ownership.
