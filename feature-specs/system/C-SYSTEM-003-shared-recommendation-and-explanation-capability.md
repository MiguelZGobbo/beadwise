# Feature Specification — C-SYSTEM-003 — Shared recommendation & explanation capability
...
**Base conceitual do Discovery:** [5] System Recommendation Engine — `CANDIDATE`; [50] Explain Why — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Shared recommendation & explanation capability**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Shared recommendation & explanation capability; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Shared recommendation & explanation capability**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SYSTEM-003
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Shared recommendation & explanation capability**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/system/c-system-003/` — **TBD / ainda não executado nesta fase documental**.
...
- Capability lógica compartilhada: recomendações devem ser derivadas de estado detectado + regra versionada + evidência + confidence, nunca de “different from default”.
- Cada recomendação precisa apontar owner, benefício esperado, risco, reversibilidade e condição de não recomendar.
- Não possui Apply próprio; delega para a feature owner.
- `PROVEN` exige testes de regras, conflitos, `nothing to optimize` e explicações consistentes.
