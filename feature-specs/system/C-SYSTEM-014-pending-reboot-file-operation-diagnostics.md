# Feature Specification — C-SYSTEM-014 — Pending reboot/file-operation diagnostics
...
**Base conceitual do Discovery:** [33] Pending Reboot Detection — `CANDIDATE`; [68] Pending File Operation Detection — `IDEA`
...
Evita decisões baseadas em suposição sobre **Pending reboot/file-operation diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Pending reboot/file-operation diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Pending reboot/file-operation diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-SYSTEM-014
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Pending reboot/file-operation diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/system/c-system-014/` — **TBD / ainda não executado nesta fase documental**.
