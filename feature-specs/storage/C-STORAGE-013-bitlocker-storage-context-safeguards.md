# Feature Specification — C-STORAGE-013 — BitLocker storage context & safeguards
...
### O que é?
Expor estado BitLocker e transições de criptografia como contexto/safeguard, sem sugerir desativação para performance.

**Base conceitual do Discovery:** [100] BitLocker State Detection — `CANDIDATE`; [101] BitLocker Encryption-in-Progress Detection — `CANDIDATE`; [102] BitLocker Performance Context — `CANDIDATE`

### Objetivo
Evitar benchmarks ou operações sensíveis durante encryption/decryption e explicar BypassIO/stack behavior afetado por BitLocker.

### Problema que resolve
Criptografia em progresso gera I/O e pode contaminar medição; desativar BitLocker por performance é trade-off de segurança inadequado como otimização automática.
...
### Quando se aplica?
Volumes BitLocker-capable; cmdlets podem depender de edição/feature.
...
- Nunca recomendar desabilitar BitLocker apenas para FPS/benchmark.
- Não alterar protectors/encryption nesta capability.
...
N/A; consumidores usam como guard.

### Motivo apresentado ao usuário
N/A

### Grau de confiança
High quando Get-BitLockerVolume/manage-bde retorna estado.

## 7. Estado atual

### O que precisa ser detectado?
- Get-BitLockerVolume primário em protótipo/runtime se módulo disponível.
- manage-bde -status como fallback.

### Como detectar?
Adapter estruturado de BitLocker volume status correlacionado a volume GUID.

### Fonte da verdade
BitLocker APIs/cmdlets para volume state.

### Estados possíveis
- Fully encrypted/protected
- Encryption in progress
- Decryption in progress
- Suspended/unprotected
- Fully decrypted
- Locked
- Unsupported/Unknown

## 8. Estado alvo
Diagnóstico; nenhuma mudança.
...
WMI BitLocker provider pode reduzir dependência de PowerShell; avaliar para backend final.

### Abordagem escolhida
Cmdlet oferece campos explícitos e é bom para prova; provider/API pode ser substituição final.
...
Alguns detalhes/operações podem exigir admin; detecção básica deve ser testada sem.
...
```text
Feature: C-STORAGE-013
Current state: resultado estruturado de Detect
Target state: Diagnóstico; nenhuma mudança.
...
- Expor key protector details desnecessários seria sensível.

### Pior cenário plausível
Vazamento de informação de recovery key; nunca coletar material secreto, apenas tipo/status.
...
### Windows
Windows 10/11; disponibilidade do BitLocker varia por edição/capability.
...
## 23. Conflitos
- Device Encryption UI may abstract BitLocker but underlying status remains authoritative.
...
Não persistir chaves/recovery material.
...
### Integração real
VM com volume encrypted, encryption-in-progress e suspended states; validar benchmark guard.
...
### Script/protótipo
Local planejado: `/prototypes/storage/c-storage-013/`

### Resultado
Detection documented; transition-state test PENDING.
...
## 31. Evidências
- bitlocker: https://learn.microsoft.com/en-us/powershell/module/bitlocker/get-bitlockervolume?view=windowsserver2025-ps
...
- Backend final: BitLocker WMI API vs PowerShell dependency.
