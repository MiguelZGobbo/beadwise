...
- Capability/premissa exercitada: Volume capacity/free space can be read without hydrating cloud placeholders.
...
- Limites preservados: no cloud-provider placeholder fixture
...
- Prova prática adicional essencial: Sim — volumes/space e cloud-placeholder safety.
- Evidência realmente executada: Prototype seguro adicional executado: Volume capacity/free space can be read without hydrating cloud placeholders. Resultado observado: `PASS`. Evidência: `/prototypes/storage/C-STORAGE-001/results/storage-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — `Get-Volume`/`Get-Item` in read-only, testar state sem placeholder.
- Impedimento ou limitação restante: Não há provider cloud/placeholder para tamanho on-disk.
