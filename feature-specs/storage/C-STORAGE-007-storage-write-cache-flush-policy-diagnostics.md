...
- Prova prática adicional essencial: Sim — write cache policy via IOCTL e significado UI.
- Evidência realmente executada: NENHUMA.
- Execução segura neste host / teste: Sim — `IOCTL_STORAGE_QUERY_PROPERTY` (`STORAGE_WRITE_CACHE_PROPERTY`) read-only/PInvoke no disco alvo.
- Impedimento ou limitação restante: Campos/UI ainda não mapeados e sem política de alteração segura.
