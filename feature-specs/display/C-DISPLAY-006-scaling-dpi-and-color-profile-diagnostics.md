# Feature Specification — C-DISPLAY-006 — Scaling, DPI & color profile diagnostics

## 3. Identificação

Nome: Scaling, DPI & color profile diagnostics
ID: C-DISPLAY-006
Tipo: Diagnostic, Diagnostic / Configuration, Diagnostic / Manual Guidance, Tool / Manual Guidance
...
**Base conceitual do Discovery:** [16] Display Scaling & DPI Diagnostics — `CANDIDATE`; [19] Color Depth & Output Capability Diagnostics — `CANDIDATE`; [20] Color Profile / ICC Awareness — `CANDIDATE`; [21] Windows Color Calibration Guidance — `CANDIDATE`; [22] Gamma / Brightness / Contrast Diagnostic Guidance — `CANDIDATE`; [45] Chroma Subsampling & Pixel Format Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Scaling, DPI & color profile diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Scaling, DPI & color profile diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Scaling, DPI & color profile diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DISPLAY-006
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Scaling, DPI & color profile diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/display/c-display-006/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Yes — per-monitor DPI/scaling and color-profile presence/absence.
- Evidência realmente executada: No direct DPI/color-profile probe.
- Execução segura neste host / teste: Read-only query is safe; distinct-DPI/profile matrix is not available. Teste recomendado: Query per-monitor DPI/scaling and WCS profile APIs; compare monitors with distinct DPI and present/absent ICC profiles.
- Impedimento ou limitação restante: Current evidence shows one generic monitor and no profile matrix; no feature prototype.
