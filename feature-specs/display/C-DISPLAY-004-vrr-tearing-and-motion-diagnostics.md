# Feature Specification — C-DISPLAY-004 — VRR, tearing & motion diagnostics

## 3. Identificação

Nome: VRR, tearing & motion diagnostics  
ID: C-DISPLAY-004  
...
**Base conceitual do Discovery:** [13] VRR Configuration & Validation — `CANDIDATE`; [31] Ghosting / Motion Clarity Test — `CANDIDATE`; [32] Tearing Visual Validation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **VRR, tearing & motion diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for VRR, tearing & motion diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **VRR, tearing & motion diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DISPLAY-004
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **VRR, tearing & motion diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/display/c-display-004/` — **TBD / ainda não executado nesta fase documental**.
...
- VRR efetivo depende de OS, driver, display, mode e aplicativo; capability flag não prova que a apresentação está usando VRR.
- Não criar toggle universal nem recomendar VRR ON/OFF para todo workload.
- Definir observabilidade end-to-end e fonte de verdade por vendor/OS; PresentMon/ETW pode medir presentation behavior, não necessariamente o painel do driver.
- `SPECIFIED` depende de separar capability, configured state e observed behavior.
