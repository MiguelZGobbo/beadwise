# Feature Specification — C-DISPLAY-012 — External monitor control & firmware awareness

## 3. Identificação

Nome: External monitor control & firmware awareness  
ID: C-DISPLAY-012  
Tipo: Configuration / Tool, Diagnostic, Diagnostic / Manual Guidance, Tool / Configuration  
...
**Base conceitual do Discovery:** [47] External Monitor DDC/CI Capability Diagnostics — `CANDIDATE`; [48] External Monitor Brightness Control — `CANDIDATE`; [49] Monitor Input Source Control — `CANDIDATE`; [50] Monitor Firmware Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **External monitor control & firmware awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for External monitor control & firmware awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **External monitor control & firmware awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DISPLAY-012
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **External monitor control & firmware awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/display/c-display-012/` — **TBD / ainda não executado nesta fase documental**.
...
- DDC/CI/MCCS e vendor utilities não são universais; nem todo monitor/connection expõe controle ou firmware update.
- Capability permanece diagnostics/vendor-awareness até existir adapter suportado por fabricante.
- Firmware update nunca deve ser genérico e exige pacote/procedimento oficial específico do modelo.
- `Unsupported` é estado normal.
