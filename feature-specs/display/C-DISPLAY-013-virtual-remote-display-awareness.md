# Feature Specification — C-DISPLAY-013 — Virtual/remote display awareness
...
**Base conceitual do Discovery:** [52] Remote Desktop / Virtual Display Awareness — `CANDIDATE`; [53] Privacy Screen / Integrated Display Feature Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Virtual/remote display awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Virtual/remote display awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Virtual/remote display awareness** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-DISPLAY-013
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Virtual/remote display awareness** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/display/c-display-013/` — **TBD / ainda não executado nesta fase documental**.
