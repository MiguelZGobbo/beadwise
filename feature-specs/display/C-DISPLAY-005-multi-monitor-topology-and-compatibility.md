# Feature Specification — C-DISPLAY-005 — Multi-monitor topology & compatibility
...
**Base conceitual do Discovery:** [14] Multi-Monitor Topology Diagnostics — `CANDIDATE`; [46] Clone Mode Capability Limitation Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Multi-monitor topology & compatibility**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Multi-monitor topology & compatibility; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Multi-monitor topology & compatibility** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-DISPLAY-005
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Multi-monitor topology & compatibility** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/display/c-display-005/` — **TBD / ainda não executado nesta fase documental**.
