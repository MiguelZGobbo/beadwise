# Feature Specification — C-DISPLAY-011 — Display change safety, rollback & validation
...
**Base conceitual do Discovery:** [38] Display Configuration Backup & Rollback — `CANDIDATE`; [39] Display Change Outcome Validation — `CANDIDATE`; [57] Display Test Flashing Safety — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Display change safety, rollback & validation**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Display change safety, rollback & validation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DISPLAY-011
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Display change safety, rollback & validation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/display/c-display-011/` — **TBD / ainda não executado nesta fase documental**.
...
- Shared safeguard para operações de modo: `SDC_VALIDATE` antes de Apply quando aplicável, snapshot CCD, timeout de confirmação na UX futura e rollback para topologia/modos originais.
- Não considerar exit code suficiente; Verify reconsulta estado efetivo.
- Se display/path desaparecer, rollback pode ser partial e deve usar configuração persistida/safe fallback documentado sem inventar modo.
- `PROVEN` exige mudança válida, inválida, monitor removido e rollback.
