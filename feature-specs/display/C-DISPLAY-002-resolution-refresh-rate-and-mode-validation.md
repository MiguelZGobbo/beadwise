# Feature Specification — C-DISPLAY-002 — Resolution, refresh rate & mode validation
...
**Base conceitual do Discovery:** [4] Native Resolution Validation — `CANDIDATE`; [5] Refresh Rate Capability & Active Rate Validation — `CANDIDATE`; [6] Dynamic Refresh Rate Awareness — `CANDIDATE`; [7] Display Mode Compatibility Diagnostics — `CANDIDATE`; [12] Variable Refresh Rate Capability Diagnostics — `CANDIDATE`; [15] Mixed Refresh Rate Multi-Monitor Awareness — `CANDIDATE`; [17] Resolution vs Scaling Recommendation Logic — `CANDIDATE`; [18] Display Orientation & Layout Validation — `CANDIDATE`; [56] Refresh Rate Measurement Validation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Resolution, refresh rate & mode validation**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Resolution, refresh rate & mode validation; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Resolution, refresh rate & mode validation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DISPLAY-002
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Resolution, refresh rate & mode validation**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
