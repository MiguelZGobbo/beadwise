# Feature Specification — C-DISPLAY-003 — HDR & advanced color diagnostics

## 3. Identificação

Nome: HDR & advanced color diagnostics  
ID: C-DISPLAY-003  
Tipo: Diagnostic, Diagnostic / Configuration, Tool / Manual Guidance  
...
**Base conceitual do Discovery:** [8] HDR Capability Detection — `CANDIDATE`; [9] HDR Configuration Diagnostics — `CANDIDATE`; [10] HDR Calibration Guidance — `CANDIDATE`; [11] SDR Content Brightness in HDR Diagnostics — `CANDIDATE`; [40] Advanced Color Capability & State Diagnostics — `CANDIDATE`; [41] Auto Color Management Awareness — `CANDIDATE`; [42] ICC Profile Compatibility Under Advanced Color — `CANDIDATE`; [43] HDR / Wide-Gamut Multi-Monitor Consistency Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **HDR & advanced color diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for HDR & advanced color diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **HDR & advanced color diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DISPLAY-003
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **HDR & advanced color diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/display/c-display-003/` — **TBD / ainda não executado nesta fase documental**.
...
- Detecção de Advanced Color/HDR é possível em versões modernas, porém toggle/configuração e semântica de SDR/HDR variam por build, GPU/driver e display.
- Não escrever Registry/UI-internal state como contrato.
- Definir API pública mínima por Windows build e provar Verify da configuração efetiva antes de `SPECIFIED`.
- Color management/ICC deve permanecer separado de HDR capability.
