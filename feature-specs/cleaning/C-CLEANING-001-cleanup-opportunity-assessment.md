# Feature Specification — C-CLEANING-001 — Cleanup opportunity assessment
...
**Base conceitual do Discovery:** [1] Storage Pressure & Recoverable Space Assessment — `CANDIDATE`; [2] Cleanup Inventory & Category Classification — `CANDIDATE`; [33] Cleanup Opportunity Ranking — `CANDIDATE`; [34] Critical Free-Space Requirement Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Cleanup opportunity assessment**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Cleanup opportunity assessment; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Cleanup opportunity assessment** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-CLEANING-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Cleanup opportunity assessment** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
