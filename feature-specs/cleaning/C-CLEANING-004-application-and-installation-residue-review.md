# Feature Specification — C-CLEANING-004 — Application & installation residue review

## 3. Identificação

Nome: Application & installation residue review  
ID: C-CLEANING-004  
Tipo: Diagnostic / Configuration, Diagnostic / Tool  
...
**Base conceitual do Discovery:** [8] Unused Application Discovery & Uninstall Guidance — `CANDIDATE`; [18] Installation Residue Review — `CANDIDATE`; [31] Orphaned User Profile Assessment — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Application & installation residue review**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Application & installation residue review**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-CLEANING-004
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Application & installation residue review**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/cleaning/c-cleaning-004/` — **TBD / ainda não executado nesta fase documental**.
