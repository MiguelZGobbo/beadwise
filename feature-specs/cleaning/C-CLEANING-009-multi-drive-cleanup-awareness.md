# Feature Specification — C-CLEANING-009 — Multi-drive cleanup awareness
...
**Base conceitual do Discovery:** [28] Multi-Drive Cleanup Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Multi-drive cleanup awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Multi-drive cleanup awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Multi-drive cleanup awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-CLEANING-009
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Multi-drive cleanup awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/cleaning/c-cleaning-009/` — **TBD / ainda não executado nesta fase documental**.
...
- Read-only: enumerar volumes/mount points, filesystem, capacidade/free space e mapear cleanup candidate ao volume real.
- Não somar hardlinks/reparse content como bytes liberáveis sem accounting correto.
- Removable/network/offline volumes precisam de estado explícito e não entram em cleanup automático.
- `PROVEN` exige múltiplos volumes e mount points não-lettered quando disponíveis.
