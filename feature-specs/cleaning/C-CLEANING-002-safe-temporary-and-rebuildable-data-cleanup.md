# Feature Specification — C-CLEANING-002 — Safe temporary & rebuildable data cleanup

## 3. Identificação

Nome: Safe temporary & rebuildable data cleanup
ID: C-CLEANING-002
Tipo: Configuration / Optimization, Diagnostic, Maintenance / Optimization, Optimization, Optimization / Repair-adjacent
...
**Base conceitual do Discovery:** [3] Safe Temporary File Cleanup — `CANDIDATE`; [4] Recycle Bin Space Management — `CANDIDATE`; [10] Previous Windows Installation Cleanup — `CANDIDATE`; [11] Windows Update Temporary Data Cleanup — `CANDIDATE`; [12] Delivery Optimization Cache Cleanup — `CANDIDATE`; [13] Windows Component Store Cleanup — `CANDIDATE`; [14] App-Owned Temporary Data Cleanup — `CANDIDATE`; [15] Rebuildable Cache Classification — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Safe temporary & rebuildable data cleanup**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- free space and candidate size before deletion
- candidate provenance/category and ownership before marking safe
- Known Folder and package-aware paths rather than hard-coded user paths
- Storage Sense policy/state awareness
- Windows servicing cleanup only through documented DISM servicing commands
- N/A
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Safe temporary & rebuildable data cleanup; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Safe temporary & rebuildable data cleanup**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
- GetDiskFreeSpaceEx
- Known Folder API
- Storage Sense Policy CSP
- DISM /StartComponentCleanup /AnalyzeComponentStore when servicing scope applies
- filesystem APIs with reparse-point and access checks
...
```text
Feature: C-CLEANING-002
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Safe temporary & rebuildable data cleanup**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
PLAN: PASS (2 allowlisted files; 40 bytes)
DRY-RUN: PASS (tree fingerprint unchanged)
APPLY: PASS (prototype-owned temp sandbox only)
VERIFY: PASS (eligible files removed; exclusions and bytes verified)
...
**Prova segura adicional observada (2026-09-08):** Execução real limitada a diretório temporário único criado pelo prototype. O plano removeu somente 2 arquivos allowlisted, confirmou 40 bytes liberados, reteve locked/protected/fresh/unknown/reparse e eliminou a fixture no `finally`. Nenhum root real do usuário ou Windows foi lido como candidato ou alterado.

Evidência: /prototypes/cleaning/C-CLEANING-007/results/.
<!-- PHASE2-SAFE-PROOF-2026-09-08:END -->
<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Yes — isolated temp cleanup with byte-identical dry-run, locked/reparse exclusions and post-delete verification.
- Evidência realmente executada: Prototype seguro adicional executado: Execução real limitada a diretório temporário único criado pelo prototype. O plano removeu somente 2 arquivos allowlisted, confirmou 40 bytes liberados, reteve locked/protected/fresh/unknown/reparse e eliminou a fixture no `finally`. Nenhum root real do usuário ou Windows foi lido como candidato ou alterado. Evidência: `/prototypes/cleaning/C-CLEANING-007/results/`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Yes only inside a unique prototype-owned temporary sandbox; never target user/system cleanup roots. Teste recomendado: Create known files, a locked file, reparse link and protected/unknown candidate in a sandbox; preview; assert dry-run no mutation; delete allowlisted files; verify bytes/state and cleanup sandbox.
- Impedimento ou limitação restante: Real user/system deletion is destructive and has no rollback; full product claim cannot be proven against live roots.
...
- Cleanup só atua em categorias allowlisted cuja propriedade/rebuildability está estabelecida; preview lista paths/categorias/bytes antes de excluir.
- Arquivo em uso, reparse point inesperado, ownership incerto ou localização protegida = skip.
- Exclusão é normalmente irreversível; rollback = No, portanto não chamar System Restore de rollback de dados.
- `PROVEN` exige dry-run byte-identical, arquivos locked e verificação pós-delete.
