# Feature Specification — C-CLEANING-008 — Cleanup policy & Storage Sense awareness

## 3. Identificação

Nome: Cleanup policy & Storage Sense awareness
ID: C-CLEANING-008
Tipo: Configuration, Diagnostic / Configuration, Optimization / Recommendation
...
**Base conceitual do Discovery:** [24] Age / Context-Aware Cleanup Policy — `CANDIDATE`; [25] Low-Disk Triggered Cleanup Recommendation — `CANDIDATE`; [26] Storage Sense Configuration Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Cleanup policy & Storage Sense awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Cleanup policy & Storage Sense awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Cleanup policy & Storage Sense awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-CLEANING-008
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Cleanup policy & Storage Sense awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/cleaning/c-cleaning-008/` — **TBD / ainda não executado nesta fase documental**.
