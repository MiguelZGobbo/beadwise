# Feature Specification — C-CLEANING-003 — User file review & space recovery
...
**Base conceitual do Discovery:** [5] Downloads Folder Review — `CANDIDATE`; [6] Large & Unused File Discovery — `CANDIDATE`; [7] Duplicate File Discovery — `CANDIDATE`; [9] Cloud Content Dehydration / Free-Up-Space — `CANDIDATE`; [19] User Data Archive / Move Recommendation — `CANDIDATE`; [32] Installer / Archive Redundancy Correlation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **User file review & space recovery**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for User file review & space recovery; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **User file review & space recovery**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-CLEANING-003
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **User file review & space recovery**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/cleaning/c-cleaning-003/` — **TBD / ainda não executado nesta fase documental**.
