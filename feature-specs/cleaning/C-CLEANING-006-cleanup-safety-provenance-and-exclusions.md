# Feature Specification — C-CLEANING-006 — Cleanup safety, provenance & exclusions
...
**Base conceitual do Discovery:** [20] Active-Use & File-Lock Safeguard — `CANDIDATE`; [21] Cleanup Ownership & Provenance Validation — `CANDIDATE`; [23] Cleanup Exclusions & Protected Locations — `CANDIDATE`; [29] Reparse Point / Hardlink / Sparse File Accounting — `CANDIDATE`; [30] Per-User / Multi-Profile Cleanup Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Cleanup safety, provenance & exclusions**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Cleanup safety, provenance & exclusions; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Cleanup safety, provenance & exclusions**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-CLEANING-006
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Cleanup safety, provenance & exclusions**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
- Capability de gate: canonicalize path, detect reparse/hardlink/sparse context, ownership, active-use e protected locations antes de qualquer cleaner.
- Default-deny quando provenance é desconhecida; regras de exclusão têm precedência sobre recomendação de espaço.
- Não segue reparse points para fora da raiz autorizada durante enumeração/delete.
- `PROVEN` exige testes com junction/symlink/hardlink, arquivos locked e múltiplos perfis.
