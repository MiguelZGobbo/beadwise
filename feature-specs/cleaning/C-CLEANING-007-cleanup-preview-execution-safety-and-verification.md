# Feature Specification — C-CLEANING-007 — Cleanup preview, execution safety & verification
...
**Base conceitual do Discovery:** [22] Cleanup Preview / Dry Run — `CANDIDATE`; [27] Cleanup Result Verification & History — `CANDIDATE`; [35] Cleanup Cancellation / Partial-Failure Safety — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Cleanup preview, execution safety & verification**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Cleanup preview, execution safety & verification; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Cleanup preview, execution safety & verification** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-CLEANING-007
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Cleanup preview, execution safety & verification** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
