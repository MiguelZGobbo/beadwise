# Feature Specification — C-PROCESSES-004 — Process responsiveness, crash & lifecycle diagnostics
...
**Base conceitual do Discovery:** [11] Process Responsiveness / Hang Diagnostics — `CANDIDATE`; [12] Process Crash History Correlation — `CANDIDATE`; [13] Process Start / Exit Monitoring — `CANDIDATE`; [30] Process Change / Action History — `CANDIDATE`; [39] Process Leak Trend Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Process responsiveness, crash & lifecycle diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Process responsiveness, crash & lifecycle diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Process responsiveness, crash & lifecycle diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-PROCESSES-004
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Process responsiveness, crash & lifecycle diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/processes/c-processes-004/` — **TBD / ainda não executado nesta fase documental**.
