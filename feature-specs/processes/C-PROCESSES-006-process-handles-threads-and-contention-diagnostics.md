# Feature Specification — C-PROCESSES-006 — Process handles, threads & contention diagnostics

## 3. Identificação

Nome: Process handles, threads & contention diagnostics
ID: C-PROCESSES-006
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [22] Process Handle / Thread Count Diagnostics — `CANDIDATE`; [40] Process Contention Correlation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Process handles, threads & contention diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Process handles, threads & contention diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Process handles, threads & contention diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-PROCESSES-006
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Process handles, threads & contention diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/processes/c-processes-006/` — **TBD / ainda não executado nesta fase documental**.
...
**Prova prática segura executada (2026-09-08):** `C-PROCESSES-006=PARTIAL` em `/prototypes/processes/C-PROCESSES-001/results/process-safe-capabilities.json`. Threads/handles foram consultados no filho próprio; a leitura do PID 4 foi registrada como `READABLE`, portanto nenhum `AccessDenied` foi inventado. Contention, deadlock, kernel handle leak e caminho real de negação continuam sem prova.
<!-- PHASE2-SAFE-PROOF:END -->

<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Sim — handles/threads/contenção e `AccessDenied`.
- Evidência realmente executada: Prototype seguro adicional executado: `C-PROCESSES-006=PARTIAL` em `/prototypes/processes/C-PROCESSES-001/results/process-safe-capabilities.json`. Threads/handles foram consultados no filho próprio; a leitura do PID 4 foi registrada como `READABLE`, portanto nenhum `AccessDenied` foi inventado. Contention, deadlock, kernel handle leak e caminho real de negação continuam sem prova. Evidência: `/prototypes/processes/C-PROCESSES-001/results/process-safe-capabilities.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — consulta threads/handles em filho próprio e tentativa somente-read de processo protegido para erro estruturado.
- Impedimento ou limitação restante: Sem cenário real de contention/kernel handle leak.
