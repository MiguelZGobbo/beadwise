# Feature Specification — C-PROCESSES-002 — Per-process resource usage & trend diagnostics

## 3. Identificação

Nome: Per-process resource usage & trend diagnostics
ID: C-PROCESSES-002
...
**Base conceitual do Discovery:** [4] Process CPU Usage Diagnostics — `CANDIDATE`; [5] Process Memory Usage Diagnostics — `CANDIDATE`; [6] Process Disk I/O Diagnostics — `CANDIDATE`; [7] Process Network Usage Diagnostics — `CANDIDATE`; [8] Process GPU Usage Diagnostics — `CANDIDATE`; [9] Process Resource Trend Monitoring — `CANDIDATE`; [23] Process CPU-Time vs Utilization Context — `CANDIDATE`; [36] Process Power / Energy Attribution — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Per-process resource usage & trend diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Per-process resource usage & trend diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Per-process resource usage & trend diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-PROCESSES-002
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Per-process resource usage & trend diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/processes/c-processes-002/` — **TBD / ainda não executado nesta fase documental**.
...
**Prova prática segura executada (2026-09-08):** `C-PROCESSES-002=PASS` em `/prototypes/processes/C-PROCESSES-001/results/process-safe-capabilities.json`. Cinco amostras temporais sobre workload filho próprio registraram CPU acumulada/delta, wall delta, working set e normalização por 16 processadores lógicos. A janela curta não prova tendência longa, GPU ou ETW sustentado.
<!-- PHASE2-SAFE-PROOF:END -->

<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Sim — amostragem temporal/overhead, não snapshot único.
- Evidência realmente executada: Prototype seguro adicional executado: `C-PROCESSES-002=PASS` em `/prototypes/processes/C-PROCESSES-001/results/process-safe-capabilities.json`. Cinco amostras temporais sobre workload filho próprio registraram CPU acumulada/delta, wall delta, working set e normalização por 16 processadores lógicos. A janela curta não prova tendência longa, GPU ou ETW sustentado. Evidência: `/prototypes/processes/C-PROCESSES-001/results/process-safe-capabilities.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — 3–5 amostras PDH/CIM sobre processo próprio e validação de delta/zero.
- Impedimento ou limitação restante: Não prova carga sustentada/ETW de longa duração.
