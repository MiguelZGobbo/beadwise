# Feature Specification — C-PROCESSES-003 — Process anomaly & root-cause correlation
...
**Base conceitual do Discovery:** [10] Foreground vs Background Context — `CANDIDATE`; [28] Process Resource Anomaly Detection — `CANDIDATE`; [37] Process Wake / Sleep Prevention Diagnostics — `CANDIDATE`; [44] Process Known-Role Knowledge Boundary — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Process anomaly & root-cause correlation**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Process anomaly & root-cause correlation; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Process anomaly & root-cause correlation** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-PROCESSES-003
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Process anomaly & root-cause correlation** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/processes/c-processes-003/` — **TBD / ainda não executado nesta fase documental**.
