# Feature Specification — C-PROCESSES-001 — Process inventory, tree & app grouping
...
**Base conceitual do Discovery:** [1] Process Inventory — `CANDIDATE`; [2] Process Tree & Parent/Child Mapping — `CANDIDATE`; [3] Application-to-Process Grouping — `CANDIDATE`; [32] Process Command-Line & Launch Context Diagnostics — `CANDIDATE`; [33] Process Session / User Isolation Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Process inventory, tree & app grouping**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Process inventory, tree & app grouping; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Process inventory, tree & app grouping** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-PROCESSES-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Process inventory, tree & app grouping** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/processes/c-processes-001/` — **TBD / ainda não executado nesta fase documental**.
