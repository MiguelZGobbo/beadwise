# Feature Specification — C-DRIVERS-006 — Driver store maintenance

## 3. Identificação

Nome: Driver store maintenance
ID: C-DRIVERS-006
Tipo: Diagnostic, Diagnostic / UX, Safeguard / Repair
...
**Base conceitual do Discovery:** [13] Driver Store Cleanup Diagnostics — `CANDIDATE`; [14] Driver Package Removal Safeguard — `CANDIDATE`; [30] Driver Store Size & Growth Diagnostics — `CANDIDATE`; [31] Duplicate / Superseded Package Analysis — `CANDIDATE`; [32] Device Removal vs Driver Package Removal Distinction — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Driver store maintenance**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Driver store maintenance; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Driver store maintenance**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DRIVERS-006
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Driver store maintenance**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/drivers/c-drivers-006/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Yes — PnPUtil inventory/export, preview, current/in-use block and removal/restore semantics.
- Evidência realmente executada: No direct probe; all operations are `NOT_TESTED`.
- Execução segura neste host / teste: Inventory/export/preview may be safe; removal and rollback are not safe without a sacrificial driver fixture/admin. Teste recomendado: Use a non-critical test driver package in a disposable VM/test machine; enumerate, export, preview, remove, verify and restore from known package.
- Impedimento ou limitação restante: Non-admin live desktop and no sacrificial package/VM; `/delete-driver` could destabilize the host.
