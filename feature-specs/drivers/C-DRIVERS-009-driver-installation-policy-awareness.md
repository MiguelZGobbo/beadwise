# Feature Specification — C-DRIVERS-009 — Driver installation policy awareness
...
**Base conceitual do Discovery:** [27] Driver Installation Policy Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Driver installation policy awareness**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Driver installation policy awareness; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Driver installation policy awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DRIVERS-009
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Driver installation policy awareness**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/drivers/c-drivers-009/` — **TBD / ainda não executado nesta fase documental**.
...
- Read-only: identificar policy/device-install restrictions quando observável e devolver `POLICY_BLOCKED/MANAGED`; não contornar GPO/MDM.
- Policy configured e effective device install result devem permanecer distintos.
- Sem Apply próprio; install workflow consome esse gate.
- `PROVEN` exige unmanaged e policy-blocked environment.
