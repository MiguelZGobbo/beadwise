# Feature Specification — C-DRIVERS-004 — Driver install, rollback & restart workflow

## 3. Identificação

Nome: Driver install, rollback & restart workflow  
ID: C-DRIVERS-004  
Tipo: Diagnostic, Diagnostic / Repair, Repair, Repair / Configuration, Safeguard  
...
**Base conceitual do Discovery:** [10] Driver Rollback Capability — `CANDIDATE`; [11] Targeted Driver Install via Official Package — `CANDIDATE`; [15] Driver Restart / Device Restart Orchestration — `CANDIDATE`; [16] Reboot-Required Driver State — `CANDIDATE`; [37] Driver Operation Dry-Run / Impact Preview — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Driver install, rollback & restart workflow**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Driver install, rollback & restart workflow; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Driver install, rollback & restart workflow**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-DRIVERS-004
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Driver install, rollback & restart workflow**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/drivers/c-drivers-004/` — **TBD / ainda não executado nesta fase documental**.
...
- PnPUtil documenta add/install/delete/export/restart, mas isso não prova um contrato determinístico para restaurar “o driver anterior” equivalente ao Device Manager Roll Back Driver.
- Antes de `SPECIFIED`, definir snapshot suficiente do pacote/driver selection, garantir que pacote anterior exista e provar rebind/rollback em hardware/VM.
- Não usar `delete-driver /force` como rollback ou cleanup universal.
- Reboot/restart-device deve ser derivado do resultado real do driver/device.
