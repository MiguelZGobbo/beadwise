# Feature Specification — C-STARTUP-005 — Startup drift, regression & before/after validation

## 3. Identificação

Nome: Startup drift, regression & before/after validation
ID: C-STARTUP-005
Tipo: Benchmark, Diagnostic
...
**Base conceitual do Discovery:** [11] Startup Before × After Validation — `CANDIDATE`; [25] Startup Configuration Drift — `CANDIDATE`; [38] Startup Regression Correlation — `CANDIDATE`
...
### Grau de confiança
High for inventory/session diffs; Medium for causal attribution.

## 7. Estado atual

### O que precisa ser detectado?
- Timestamped normalized startup inventory snapshots from C-STARTUP-001 including source-specific identity and enabled/transient state.
- Startup performance sessions from C-STARTUP-002 with boot mode, Windows build, power context and measurement method.
- Change session records from C-STARTUP-008, including exact entries modified and verification results.
- External drift: entry added/removed/changed outside BeadWise between snapshots.
- Regression correlation: new/changed entries occurring in the same interval as worsened startup metrics; report correlation strength, never causal certainty from timing alone.

### Como detectar?
Persist normalized hashes/identity rather than raw volatile strings where possible. Compare semantic entry state and measurement distributions. A single faster/slower boot is not sufficient proof.

### Fonte da verdade
BeadWise versioned snapshots/change sessions plus measurement records. Current Windows state remains authoritative; history explains how it changed.

### Estados possíveis
- Stable
- Drift detected
- BeadWise change correlated
- External change correlated
- Performance improved/worsened/inconclusive
- Samples incomparable
- Insufficient history

## 8. Estado alvo
A before/after record that can answer what changed, whether startup metrics changed under comparable conditions, and how confident the correlation is.
...
- Outputs of C-STARTUP-001/C-STARTUP-002/C-STARTUP-008
- Local versioned persistence; no external service required

### Alternativas avaliadas
Windows Event Log alone is insufficient as a complete startup configuration history. File timestamps alone do not establish causal drift.
...
```text
Feature: C-STARTUP-005
Current state: structured Detect result
Target state: A before/after record that can answer what changed, whether startup metrics changed under comparable conditions, and how confident the correlation is.
Changes:
...
### APIs
Outputs of C-STARTUP-001/C-STARTUP-002/C-STARTUP-008, Local versioned persistence; no external service required
...
### Script/protótipo
Local: `/prototypes/startup/c-startup-005/` — TBD.
...
- Capability/premissa exercitada: Snapshot comparison detects enablement change and treats identical capture as no drift.
...
- Limites preservados: fixture drift; no boot-impact measurement
...
- Prova prática adicional essencial: Sim — snapshots/diff/before-after e confidence.
- Evidência realmente executada: Prototype seguro adicional executado: Snapshot comparison detects enablement change and treats identical capture as no drift. Resultado observado: `PASS`. Evidência: `/prototypes/startup/C-STARTUP-006/results/startup-contract.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — duas capturas + entrada temporária em HKCU/Startup folder owned by prototype; Verify e rollback serializados.
- Impedimento ou limitação restante: Não mede boot impact real.
...
### Como?
Compare repeated like-for-like startup session metrics and report absolute/relative delta plus variance; no universal “good” threshold.
...
- Define minimum sample-count/confidence policy for before/after claims after prototype data is available.
