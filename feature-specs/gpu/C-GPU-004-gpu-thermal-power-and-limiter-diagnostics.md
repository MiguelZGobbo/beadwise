# Feature Specification — C-GPU-004 — GPU thermal, power & limiter diagnostics

## 3. Identificação

Nome: GPU thermal, power & limiter diagnostics  
ID: C-GPU-004  
Tipo: Advanced Tuning, Diagnostic, Monitoring, Validation  
...
**Base conceitual do Discovery:** [24] GPU Temperature Monitoring — `CANDIDATE`; [25] GPU Hotspot Temperature — `RESEARCHING`; [26] GPU Memory Temperature — `RESEARCHING`; [31] GPU Power Limit Detection — `CANDIDATE`; [32] GPU Thermal Limit Detection — `CANDIDATE`; [33] GPU Voltage / Reliability Limiter Context — `RESEARCHING`; [34] GPU Performance Limiter Classification — `CANDIDATE`; [101] GPU Temperature Stability Validation — `CANDIDATE`; [131] GPU Power Limit Modification — `QUESTIONABLE`; [152] GPU Limiter Diagnostics — `CANDIDATE`; [153] Laptop GPU Power Envelope Context — `CANDIDATE`; Limite conceitual:; O `READY_FOR_SPEC` vale para a responsabilidade principal descrita nesta capability. Subitens de origem marcados `QUESTIONABLE` continuam questionáveis e **não** são aprovados implicitamente; qualquer alteração agressiva neles representada permanece fora do escopo ativo até justificativa própria.
...
Evita decisões baseadas em suposição sobre **GPU thermal, power & limiter diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- Temperature/limit fields are accepted only from OS/vendor sources that identify sensor semantics; unknown sensor mapping remains Unknown.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for GPU thermal, power & limiter diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **GPU thermal, power & limiter diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **GPU thermal, power & limiter diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/gpu/c-gpu-004/` — **TBD / ainda não executado nesta fase documental**.
