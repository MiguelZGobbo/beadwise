# Feature Specification — C-GPU-008 — Per-app GPU selection & hybrid graphics routing

## 3. Identificação

Nome: Per-app GPU selection & hybrid graphics routing  
ID: C-GPU-008  
Tipo: Configuration, Diagnostic, Monitoring, Recommendation  
...
**Base conceitual do Discovery:** [61] Per-App GPU Preference Detection — `CANDIDATE`; [62] Per-App GPU Preference Configuration — `CANDIDATE`; [63] Wrong-GPU Execution Detection — `CANDIDATE`; [64] Hybrid Graphics Topology — `CANDIDATE`; [65] Hybrid Graphics Runtime Usage — `CANDIDATE`; [66] dGPU Idle Residency Diagnosis — `IDEA`; [67] External Display GPU Routing Context — `IDEA`; [68] MUX Capability Detection — `RESEARCHING`; [69] MUX Mode Advisor — `IDEA`; [154] Hybrid Graphics Routing Diagnostics — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Per-app GPU selection & hybrid graphics routing**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Per-app GPU selection & hybrid graphics routing; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Per-app GPU selection & hybrid graphics routing**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Per-app GPU selection & hybrid graphics routing**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/gpu/c-gpu-008/` — **TBD / ainda não executado nesta fase documental**.
...
- Windows preference/hybrid routing pode ser vendor/OS managed; API pública persistente para consumer Apply precisa prova e rollback.
