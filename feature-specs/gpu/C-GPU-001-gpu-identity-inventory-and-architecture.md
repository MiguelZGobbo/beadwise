# Feature Specification — C-GPU-001 — GPU identity, inventory & architecture

## 3. Identificação

Nome: GPU identity, inventory & architecture
ID: C-GPU-001
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [1] GPU Adapter Inventory — `CANDIDATE`; [2] GPU Vendor Detection — `CANDIDATE`; [3] GPU Hardware ID Detection — `CANDIDATE`; [4] GPU Model Identification — `CANDIDATE`; [5] GPU Architecture Detection — `RESEARCHING`; [6] GPU Integrated / Discrete Detection — `CANDIDATE`; [7] Detachable / External GPU Detection — `CANDIDATE`; [8] Physical GPU Count — `CANDIDATE`; [9] GPU Engine Enumeration — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **GPU identity, inventory & architecture**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- GPU/adapter identity and memory-budget information through DXGI/DXCore where supported
- WDDM and device/driver metadata through SetupAPI/CIM as contextual evidence
- GPU engine utilization through documented GPU performance counters/ETW
- TDR and graphics-kernel incidents through Event Log/ETW
- vendor telemetry for clocks/temperature/power only behind vendor-specific adapters with explicit support
- N/A
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for GPU identity, inventory & architecture; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **GPU identity, inventory & architecture** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
- DXGI / IDXGIFactory / IDXGIAdapter
- IDXGIAdapter3::QueryVideoMemoryInfo
- DXCore where applicable
- GPU Engine performance counters / PDH
- ETW/GPUView
- SetupAPI
- vendor API adapter (conditional)
...
Target state: A normalized, provenance-aware assessment of **GPU identity, inventory & architecture** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### APIs
DXGI / IDXGIFactory / IDXGIAdapter, IDXGIAdapter3::QueryVideoMemoryInfo, DXCore where applicable, GPU Engine performance counters / PDH, ETW/GPUView, SetupAPI, vendor API adapter (conditional)

### Componentes do Windows
Componentes do subsistema GPU e infraestrutura comum de Event Log/ETW/CIM quando aplicável.
...
### Script/protótipo
Local: `/prototypes/gpu/c-gpu-001/` — **TBD / ainda não executado nesta fase documental**.
...
**Prova prática segura executada (2026-09-08):** `C-GPU-001=PASS` em `/prototypes/gpu/C-GPU-001/results/gpu-dxgi-proof.json`. DXGI enumerou a Radeon RX 570 e o Microsoft Basic Render Driver; CIM enumerou um controller e houve um match exato de nome. Nenhum estado gráfico foi alterado. Casos sem GPU, multi-GPU física e ARM64 continuam sem cobertura.
<!-- PHASE2-SAFE-PROOF:END -->

<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Sim — identidade DXGI/PnP e normalização, inclusive fallback.
- Evidência realmente executada: Prototype seguro adicional executado: `C-GPU-001=PASS` em `/prototypes/gpu/C-GPU-001/results/gpu-dxgi-proof.json`. DXGI enumerou a Radeon RX 570 e o Microsoft Basic Render Driver; CIM enumerou um controller e houve um match exato de nome. Nenhum estado gráfico foi alterado. Casos sem GPU, multi-GPU física e ARM64 continuam sem cobertura. Evidência: `/prototypes/gpu/C-GPU-001/results/gpu-dxgi-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — enumeração DXGI 1.6 por P/Invoke e comparação com `Win32_VideoController`/PnP.
- Impedimento ou limitação restante: Não cobre GPU ausente/múltipla/ARM64.
