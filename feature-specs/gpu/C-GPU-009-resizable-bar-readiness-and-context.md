# Feature Specification — C-GPU-009 — Resizable BAR readiness & context

## 3. Identificação

Nome: Resizable BAR readiness & context
ID: C-GPU-009
Tipo: Configuration Guidance, Diagnostic, Explainability
...
**Base conceitual do Discovery:** [70] Resizable BAR Capability Detection — `CANDIDATE`; [71] Resizable BAR Active-State Detection — `CANDIDATE`; [72] Resizable BAR Platform Readiness — `CANDIDATE`; [73] Resizable BAR Manual Guidance — `CANDIDATE`; [74] Intel Arc ReBAR Requirement Detection — `CANDIDATE`; [75] ReBAR Benefit Context — `CANDIDATE`; [122] AMD Smart Access Memory Detection — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Resizable BAR readiness & context**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Resizable BAR readiness & context; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Resizable BAR readiness & context**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **Resizable BAR readiness & context**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/gpu/c-gpu-009/` — **TBD / ainda não executado nesta fase documental**.
...
- PCI capability/readiness não basta para provar ReBAR ativo/utilizado cross-vendor. Definir truth source AMD/NVIDIA/Intel and unsupported.
