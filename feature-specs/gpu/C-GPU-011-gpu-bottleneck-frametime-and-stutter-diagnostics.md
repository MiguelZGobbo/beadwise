# Feature Specification — C-GPU-011 — GPU bottleneck, frametime & stutter diagnostics
...
**Base conceitual do Discovery:** [81] GPU Bottleneck Detection — `CANDIDATE`; [82] CPU-Limited GPU Underutilization Detection — `CANDIDATE`; [83] Frame Cap / VSync Limited Detection — `IDEA`; [84] GPU Performance Headroom — `IDEA`; [85] GPU Frametime Correlation — `CANDIDATE`; [87] Shader Compilation Stutter Context — `IDEA`; [91] GPU API Workload Identification — `IDEA`; [92] GPU Compute Workload Detection — `CANDIDATE`; [93] Video Decode Utilization — `CANDIDATE`; [94] Video Encode Utilization — `CANDIDATE`; [95] GPU Copy Engine Utilization — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **GPU bottleneck, frametime & stutter diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for GPU bottleneck, frametime & stutter diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **GPU bottleneck, frametime & stutter diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **GPU bottleneck, frametime & stutter diagnostics** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/gpu/c-gpu-011/` — **TBD / ainda não executado nesta fase documental**.
