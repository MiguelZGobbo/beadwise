# Feature Specification — C-GPU-015 — GPU vendor capability & profile layer
...
**Base conceitual do Discovery:** [106] NVIDIA Capability Layer — `CANDIDATE`; [107] AMD GPU Capability Layer — `CANDIDATE`; [108] Intel GPU Capability Layer — `CANDIDATE`; [109] Vendor Sensor API Abstraction — `CANDIDATE`; [110] NVIDIA Driver Profile Inventory — `CANDIDATE`; [111] NVIDIA Profile Change Detection — `IDEA`; [112] NVIDIA Threaded Optimization State — `RESEARCHING`; [114] NVIDIA Power Management Mode — `RESEARCHING`; [115] NVIDIA Low Latency Mode — `RESEARCHING`; [116] NVIDIA Reflex Awareness — `CANDIDATE`; [117] NVIDIA Frame Rate Limiter Context — `IDEA`; [118] AMD Driver Profile Inventory — `CANDIDATE`; [119] AMD Anti-Lag Context — `RESEARCHING`; [120] AMD Radeon Chill Context — `CANDIDATE`; [121] AMD Enhanced Sync Context — `RESEARCHING`; [123] Intel Arc Driver Profile Inventory — `RESEARCHING`; [124] Intel Arc Performance Readiness — `CANDIDATE`; [151] Vendor Performance Telemetry Layer — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **GPU vendor capability & profile layer**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for GPU vendor capability & profile layer; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **GPU vendor capability & profile layer**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **GPU vendor capability & profile layer**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
- Vendor capability layer needs official SDK versioning/licensing/feature mapping and fallback; no private driver interfaces.
