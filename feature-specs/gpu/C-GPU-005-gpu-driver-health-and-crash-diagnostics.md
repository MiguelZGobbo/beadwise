# Feature Specification — C-GPU-005 — GPU driver health & crash diagnostics

## 3. Identificação

Nome: GPU driver health & crash diagnostics  
ID: C-GPU-005  
Tipo: Advanced Diagnostic, Diagnostic, Recommendation, Repair / Guidance, Safety, Validation  
...
**Base conceitual do Discovery:** [35] GPU Driver Version Detection — `CANDIDATE`; [36] GPU Driver Provider Detection — `CANDIDATE`; [37] WDDM Version Detection — `CANDIDATE`; [38] Graphics Driver Model Capability Check — `CANDIDATE`; [39] GPU Driver Age Context — `IDEA`; [40] GPU Driver Update Recommendation — `CANDIDATE`; [41] GPU Driver Regression Detection — `IDEA`; [42] GPU Driver Clean Install Advisor — `RESEARCHING`; [43] Display Driver Device Error Detection — `CANDIDATE`; [44] Graphics Driver Crash History — `CANDIDATE`; [45] TDR Event Detection — `CANDIDATE`; [46] TDR Frequency Analysis — `CANDIDATE`; [47] TDR Context Correlation — `IDEA`; [48] TdrDelay Tweak Guard — `CANDIDATE`; [49] GPU Crash Dump / Live Dump Awareness — `IDEA`; [100] GPU WHEA/TDR Monitoring During Test — `CANDIDATE`; [134] GPU Optimization Rejection on TDR — `CANDIDATE`; [148] GPU Device Removed Diagnostics — `CANDIDATE`; [149] DRED Crash Context — `CANDIDATE`; [150] GPU Crash Incident Correlation — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **GPU driver health & crash diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for GPU driver health & crash diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **GPU driver health & crash diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
Target state: Only an explicitly selected, supported change for **GPU driver health & crash diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/gpu/c-gpu-005/` — **TBD / ainda não executado nesta fase documental**.
...
- DRED/TDR/event evidence pode diagnosticar, mas causalidade driver vs hardware/app exige correlação; repair delegated Drivers.
