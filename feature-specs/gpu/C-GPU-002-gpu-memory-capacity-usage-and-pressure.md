# Feature Specification — C-GPU-002 — GPU memory capacity, usage & pressure

## 3. Identificação

Nome: GPU memory capacity, usage & pressure
ID: C-GPU-002
...
**Base conceitual do Discovery:** [10] Dedicated VRAM Detection — `CANDIDATE`; [11] Dedicated System Graphics Memory Detection — `CANDIDATE`; [12] Shared System Graphics Memory Detection — `CANDIDATE`; [13] VRAM Usage Monitoring — `CANDIDATE`; [14] Shared GPU Memory Monitoring — `CANDIDATE`; [15] VRAM Pressure Detection — `CANDIDATE`; [16] VRAM Spillover Detection — `CANDIDATE`; [17] Per-Process GPU Memory Usage — `CANDIDATE`; [86] VRAM Pressure / Frametime Correlation — `CANDIDATE`; [140] GPU Video Memory Budget Diagnostics — `CANDIDATE`; [141] GPU Memory Residency Pressure Analysis — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **GPU memory capacity, usage & pressure**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for GPU memory capacity, usage & pressure; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **GPU memory capacity, usage & pressure** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **GPU memory capacity, usage & pressure** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/gpu/c-gpu-002/` — **TBD / ainda não executado nesta fase documental**.
...
**Prova prática segura executada (2026-09-08):** `C-GPU-002=PASS` para consulta instantânea em `/prototypes/gpu/C-GPU-001/results/gpu-dxgi-proof.json`. `IDXGIAdapter3::QueryVideoMemoryInfo` retornou budget, uso corrente e reservation para os adapters enumerados. O resultado não prova pressão sustentada, eviction ou comportamento sob workload e não é promoção para `PROVEN`.
<!-- PHASE2-SAFE-PROOF:END -->

<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Sim — orçamento/uso de memória, não somente `AdapterRAM`.
- Evidência realmente executada: Prototype seguro adicional executado: `C-GPU-002=PASS` para consulta instantânea em `/prototypes/gpu/C-GPU-001/results/gpu-dxgi-proof.json`. `IDXGIAdapter3::QueryVideoMemoryInfo` retornou budget, uso corrente e reservation para os adapters enumerados. O resultado não prova pressão sustentada, eviction ou comportamento sob workload e não é promoção para `PROVEN`. Evidência: `/prototypes/gpu/C-GPU-001/results/gpu-dxgi-proof.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — `IDXGIAdapter3::QueryVideoMemoryInfo` para adapter atual e tratamento de `DXGI_ERROR_UNSUPPORTED`.
- Impedimento ou limitação restante: CIM não prova uso/pressão nem budget.
