# Feature Specification — C-GPU-003 — GPU utilization, clocks & telemetry

## 3. Identificação

Nome: GPU utilization, clocks & telemetry
ID: C-GPU-003
...
**Base conceitual do Discovery:** [18] GPU Utilization Monitoring — `CANDIDATE`; [19] Per-Engine GPU Utilization — `CANDIDATE`; [20] GPU Usage by Process — `CANDIDATE`; [21] GPU Clock Monitoring — `CANDIDATE`; [22] GPU Memory Clock Monitoring — `CANDIDATE`; [23] Effective GPU Clock Analysis — `RESEARCHING`; [27] GPU Fan Speed Monitoring — `CANDIDATE`; [28] GPU Fan Stop State — `IDEA`; [29] GPU Board Power Monitoring — `CANDIDATE`; [30] GPU Voltage Monitoring — `RESEARCHING`
...
Evita decisões baseadas em suposição sobre **GPU utilization, clocks & telemetry**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for GPU utilization, clocks & telemetry; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **GPU utilization, clocks & telemetry** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
Target state: A normalized, provenance-aware assessment of **GPU utilization, clocks & telemetry** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/gpu/c-gpu-003/` — **TBD / ainda não executado nesta fase documental**.
...
- Prova prática adicional essencial: Sim — telemetria de clocks/utilização e precisão da fonte.
- Evidência realmente executada: E:gpu.identity-and-memory.cim PASS (identidade somente).
- Execução segura neste host / teste: Parcial — amostrar `GPU Engine(*)`/DXGI; registrar ausência de clock.
- Impedimento ou limitação restante: API AMD/ADL documentada e workload para validar clocks não foram encontrados/testados.
