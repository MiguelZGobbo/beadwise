# Feature Specification — C-SECURITY-008 — Security policy, tamper & unsafe-tweak diagnostics
...
**Base conceitual do Discovery:** [16] Security Protection Tamper / Policy Awareness — `CANDIDATE`; [17] Potentially Unsafe Security Tweaks Detection — `CANDIDATE`; [19] Security Provider Conflict Detection — `CANDIDATE`; [20] Protection Feature Compatibility Assessment — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Security policy, tamper & unsafe-tweak diagnostics**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Security policy, tamper & unsafe-tweak diagnostics; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Security policy, tamper & unsafe-tweak diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SECURITY-008
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Security policy, tamper & unsafe-tweak diagnostics**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/security/c-security-008/` — **TBD / ainda não executado nesta fase documental**.
