# Feature Specification — C-SECURITY-001 — Windows security baseline

## 3. Identificação

Nome: Windows security baseline
ID: C-SECURITY-001
Tipo: Diagnostic
...
**Base conceitual do Discovery:** [1] Windows Security Baseline Inventory — `CANDIDATE`; [23] Security Baseline Drift History — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Windows security baseline**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
- Windows Security/Defender state through documented Defender cmdlets/APIs where available
- Windows Firewall profile/rule state through NetSecurity / Windows Filtering Platform context
- Secure Boot and TPM state through documented platform interfaces/cmdlets
- VBS/Device Guard/Credential Guard state through documented CIM/policy surfaces
- BitLocker volume protection state through documented BitLocker interfaces
- security policy/MDM/Group Policy managed-state detection before interpreting settings
- Never recommend disabling a protection for marginal performance; managed/enforced states are authoritative constraints.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Windows security baseline; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Windows security baseline** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
- Get-MpComputerStatus / Defender interfaces
- Get-NetFirewallProfile
- Confirm-SecureBootUEFI
- Get-Tpm
- Win32_DeviceGuard / documented Device Guard surfaces
- Get-BitLockerVolume / manage-bde
- Policy CSP / Group Policy mappings
...
```text
Feature: C-SECURITY-001
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Windows security baseline** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### APIs
Get-MpComputerStatus / Defender interfaces, Get-NetFirewallProfile, Confirm-SecureBootUEFI, Get-Tpm, Win32_DeviceGuard / documented Device Guard surfaces, Get-BitLockerVolume / manage-bde, Policy CSP / Group Policy mappings

### Componentes do Windows
Componentes do subsistema SECURITY e infraestrutura comum de Event Log/ETW/CIM quando aplicável.
...
### Script/protótipo
Local: `/prototypes/security/c-security-001/` — **TBD / ainda não executado nesta fase documental**.
...
**Observed behavior (campanha 2026-09-08):** probes compartilhados read-only executados neste host: `security.defender-status`=PASS, `security.firewall-profiles`=PASS. Evidência: `/prototypes/system/C-SYSTEM-017/results/domain-evidence.json`. Estes sinais são parciais e não satisfazem, sozinhos, o gate DETECT completo desta feature.

<!-- PHASE2-SAFE-PROOF:START -->
**Prova prática segura executada (2026-09-08):** `C-SECURITY-001=PASS` para o baseline deste host em `/prototypes/security/C-SECURITY-001/results/security-readonly.json`. Foram consultados Defender, firewall, SecurityCenter2 e presença de policies no Registry, com `mutationAttempted=false`. Provider terceiro, MDM e estado alternativo de tamper não foram induzidos; o PASS do probe não fecha a matriz integral.
<!-- PHASE2-SAFE-PROOF:END -->

<!-- PHASE2-FINAL-SPECIFIED-AUDIT:START -->
**Auditoria final da necessidade de prova (2026-09-08):**
- Prova prática adicional essencial: Sim — baseline effective/managed e não-redução de proteção.
- Evidência realmente executada: Prototype seguro adicional executado: `C-SECURITY-001=PASS` para o baseline deste host em `/prototypes/security/C-SECURITY-001/results/security-readonly.json`. Foram consultados Defender, firewall, SecurityCenter2 e presença de policies no Registry, com `mutationAttempted=false`. Provider terceiro, MDM e estado alternativo de tamper não foram induzidos; o PASS do probe não fecha a matriz integral. Evidência: `/prototypes/security/C-SECURITY-001/results/security-readonly.json`. A evidência anterior foi substituída por esta observação mais recente.
- Execução segura neste host / teste: Sim — consultas Defender/firewall/Windows Security e fixtures Managed/Unknown, sem Set.
- Impedimento ou limitação restante: Sem provider de terceiros/MDM/tamper case.
