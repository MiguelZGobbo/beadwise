# Feature Specification — C-SECURITY-002 — Endpoint protection & firewall state

## 3. Identificação

Nome: Endpoint protection & firewall state
ID: C-SECURITY-002
...
**Base conceitual do Discovery:** [2] Antivirus / Defender Protection State — `CANDIDATE`; [3] Firewall Profile & Protection State — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Endpoint protection & firewall state**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Endpoint protection & firewall state; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Endpoint protection & firewall state**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SECURITY-002
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Endpoint protection & firewall state**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/security/c-security-002/` — **TBD / ainda não executado nesta fase documental**.
...
- https://learn.microsoft.com/windows/security/operating-system-security/system-security/windows-defender-security-center/
...
- Detect pode usar superfícies documentadas do Windows Security/Defender/Firewall, porém a parte `Configuration` continua sem contrato único e seguro para todos os provedores de endpoint protection.
- Separar estado do Microsoft Defender, perfis do Windows Firewall e presença de AV de terceiros; não assumir que Defender desativado = desprotegido quando outro provider está registrado.
- Tamper Protection/política corporativa pode impedir mutação; não contornar proteção nem escrever Registry interno.
- Antes de `SPECIFIED`, definir quais mudanças, se alguma, pertencem realmente a esta feature e quais devem apenas abrir/encaminhar para Windows Security.
