# Feature Specification — C-SECURITY-007 — Security update & support exposure
...
**Base conceitual do Discovery:** [15] Security Update Exposure Assessment — `CANDIDATE`; [22] Unsupported OS Security Risk — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Security update & support exposure**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Security update & support exposure; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Security update & support exposure**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SECURITY-007
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Security update & support exposure**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/security/c-security-007/` — **TBD / ainda não executado nesta fase documental**.
