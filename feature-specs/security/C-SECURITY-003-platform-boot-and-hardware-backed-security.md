# Feature Specification — C-SECURITY-003 — Platform boot & hardware-backed security
...
**Base conceitual do Discovery:** [4] Secure Boot Capability & State — `CANDIDATE`; [5] TPM Capability & Health — `CANDIDATE`; [7] Kernel DMA Protection State — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Platform boot & hardware-backed security**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Platform boot & hardware-backed security; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
A normalized, provenance-aware assessment of **Platform boot & hardware-backed security** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
...
```text
Feature: C-SECURITY-003
Current state: normalized Detect result + provenance
Target state: A normalized, provenance-aware assessment of **Platform boot & hardware-backed security** with explicit Supported/Partial/Unknown/Not Applicable states and no persistent system change.
Changes:
...
### Script/protótipo
Local: `/prototypes/security/c-security-003/` — **TBD / ainda não executado nesta fase documental**.
