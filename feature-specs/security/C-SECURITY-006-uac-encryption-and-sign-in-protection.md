# Feature Specification — C-SECURITY-006 — UAC, encryption & sign-in protection
...
**Base conceitual do Discovery:** [10] UAC Protection State — `CANDIDATE`; [12] BitLocker / Device Encryption State — `CANDIDATE`; [13] Windows Hello / Sign-in Protection Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **UAC, encryption & sign-in protection**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for UAC, encryption & sign-in protection; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **UAC, encryption & sign-in protection**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SECURITY-006
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **UAC, encryption & sign-in protection**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
