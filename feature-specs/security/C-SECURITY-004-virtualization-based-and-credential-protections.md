# Feature Specification — C-SECURITY-004 — Virtualization-based & credential protections

## 3. Identificação

Nome: Virtualization-based & credential protections  
ID: C-SECURITY-004  
...
**Base conceitual do Discovery:** [6] Core Isolation / Memory Integrity State — `CANDIDATE`; [14] LSA Protection / Credential Guard Awareness — `CANDIDATE`
...
Evita decisões baseadas em suposição sobre **Virtualization-based & credential protections**, modelando estado real, contexto, limitações e falhas antes de qualquer recomendação ou ação.
...
### Fonte da verdade
Primary truth is the documented Windows/API state closest to the subsystem being modeled for Virtualization-based & credential protections; secondary sources may enrich context but must carry provenance/confidence. Vendor data is authoritative only for vendor-defined telemetry on explicitly supported hardware.
...
## 8. Estado alvo
Only an explicitly selected, supported change for **Virtualization-based & credential protections**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
...
```text
Feature: C-SECURITY-004
Current state: normalized Detect result + provenance
Target state: Only an explicitly selected, supported change for **Virtualization-based & credential protections**, built from current state and context, reaches its declared post-condition; otherwise no change is performed.
Changes:
...
### Script/protótipo
Local: `/prototypes/security/c-security-004/` — **TBD / ainda não executado nesta fase documental**.
